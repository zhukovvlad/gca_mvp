...
# Страж документации: указатель §12, ревизии, архив и маршрутизатор §11 →
# docs/pitfalls/. Read-only, без БД, идёт секунды.
#
# Это и есть команда для правок только в `docs/` и в `AGENTS.md` — они
# освобождены от кодовых цепочек, но не от стража (§9.3, ревизия v6.18).
# В GitHub Actions тело рецепта зеркалится прямой командой: `just` не зовёт ни
# один шаг обоих workflow, и его установка дала бы новый способ упасть ДО стража.
check-agents-index:
...
#
# ci-lock-backend и check-agents-index идут ДО развилки: оба секундные, и
# ловить расхождение лока или дрейф документации после семи минут тестов
# незачем. Зависимости `just` идут по порядку, и ненулевой код любой из них
# роняет `ci` до тела рецепта.
#
# Все проверки: бэкенд- и фронт-цепочки параллельно. Обязателен перед пушем.
ci: ci-lock-backend check-agents-index
