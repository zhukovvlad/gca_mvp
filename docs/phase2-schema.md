...
**Частичные индексы каталога и import_jobs в этот список не входят намеренно**,
хотя §11 упоминает их рядом с raw SQL: `Index(..., postgresql_where=...)`
Alembic выражает декларативно, такие индексы остаются в `Base.metadata` и
видимы для `alembic check` — в отличие от трёх действительно невыразимых
объектов выше. Raw SQL здесь был бы шагом назад: индекс выпал бы из детектора
дрейфа без необходимости.
