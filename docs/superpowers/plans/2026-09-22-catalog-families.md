...
- **Происхождение семьи — четыре входа, и четвёртый обязателен:** назначенная
  ...
  (`docs/pitfalls/db.md`).
- **Границы, которые обязаны ПРОХОДИТЬ, а не падать:** архивный контекст по
  умолчанию сосуществует с действующим (в предикате частичного индекса есть
  ...
  `draft` и `archived` сосуществуют. Каждая из трёх краснела бы в обратную
  сторону, если бы ограничение было записано полным вместо частичного.
- **Корзина без статьи — одна на строку** (спека §2.2): две корзины без статьи у
  ОДНОЙ строки отвергаются — граница `COALESCE(work_category_id,-1)`, которую
  обычный `UNIQUE` пропустил бы (`NULL`-ы различны); у РАЗНЫХ строк они
  сосуществуют. *Поправка по ходу реализации (ревью задачи 1): прежняя редакция
  относила первый вход к проходящим, вопреки спеке §2.2 и §6.*
...
- **Журнал:** событие с двумя предметами и событие без предмета — обе границы
  ...
  отвергается. Отдельными входами — `context_family_assigned` с `family_id`
  отвергается, а он же с `context_id` **проходит** (обычная пара «тип ↔
  предмет»).
- **Предмет опирается на множество, а не на имя — и доказывается это вне
  таблицы.** На вставке в таблицу множество и префикс `family_%` неразличимы:
  исторический дефект давало событие `family_assigned` с предметом-контекстом,
  его переименовали в `context_family_assigned`, а любой другой тип вне списка
  раньше отвергает `CHECK` закрытого списка. Поэтому выражение ограничения
  `ck_semantic_events_subject_by_type` — **в том виде, в каком оно стоит в
  базе** (`pg_get_constraintdef`), а не константа миграции, которую ограничение
  могло бы и не использовать, — вычисляется **само по себе** над строкой `VALUES`:
  `family_assigned` с предметом-контекстом даёт `TRUE` (префикс дал бы `FALSE`),
  плюс сверка множества семейных типов с независимо записанным списком.
  *Поправка по ходу реализации (ревью задачи 1): прежняя редакция объявляла
  свидетелем вход, который префикс обрабатывает так же, как множество, — снятие
  множества не красило ни одного теста.*
...
- Каждая операция пишет своё событие: `context_split`, `context_merged`,
  `members_moved`, `context_archived`. **Разделение пишет ДВА события:**
  `context_split` на новом контексте и `context_created` с `origin = 'split'`
  (спека §2.14: `split` входит в множество `origin`, и создание контекста
  разделением — такое же создание, как на импорте).
- **`routing_rules_dropped` эта задача не пишет:** ни одна её операция правил не
  отбрасывает — слияние переводит правила на цель, архивирование при входящих
  правилах отказывает. В фиче 1 правила отбрасывает только слияние в Review, и
  событие пишет задача 9; множество `reason` у него — `{review_merge}`
  (задача 2). Операций «снять» и «переставить правило» план не заводит: таблица
  блокировок спеки §2.8 называет их в протоколе, но ни §2.4 (три операции
  оператора), ни экран §2.10 их не содержат. Следствие, которое называет devlog:
  контекст, в который ведёт правило, архивируется только слиянием. *Поправка по
  ходу реализации (ревью задачи 2): прежняя редакция относила
  `routing_rules_dropped` к задаче 6 и не называла `context_created` у
  разделения.*
...
- **Журнал шага 3 называется поимённо:** отброшенные правила — событие
  `routing_rules_dropped` с `bucket_id`, `count` и `reason = 'review_merge'`
  (множество задачи 2); архивирование исходных контекстов — `context_archived`
  с `reason = 'review_merge'`; перенос членств — `members_moved` с
  `reason = 'review_merge'`. Вход без правил в исходной корзине события
  `routing_rules_dropped` не пишет (`count` у него не бывает нулём — пустой
  аудит). *Поправка по ходу реализации (ревью задачи 2): прежняя редакция
  говорила «с записью в журнал», не называя события.*
- **Предмет `routing_rules_dropped` — контекст** (спека §2.14), а правила висят
  на корзине: предметом ставится действующий контекст по умолчанию исходной
  корзины — тот, что был им до слияния (у корзины он есть всегда, иначе
  маршрутизация отказывает). Решение оркестратора по ревью задачи 2.
