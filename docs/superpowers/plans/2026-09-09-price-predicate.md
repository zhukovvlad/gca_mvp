...
проверка нулевой суммы, четвёртой или девятой. Вход этого состояния при той же
нулевой сумме предъявлен (DoD 9 требует именно предъявления шести соседей), но
стережёт он состояние, а не порядок, и назван в тесте так.
