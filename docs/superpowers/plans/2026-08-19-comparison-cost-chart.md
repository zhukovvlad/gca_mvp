...
- [ ] **Пришло из задачи 11 (DoD 28а, вторая половина):** ВЫБОР РЯДА ось не
      ...
      чек-листе (найдено ревью задачи 11).
- [ ] **Пришло из задачи 10:** клик по чипу класса не должен ронять фокус
      клавиатуры и гасить всю панель скелетоном. `placeholderData:
      keepPreviousData` в `useComparison` — правка состояний загрузки ВСЕГО
      экрана, поэтому со своим тестом (фокус остаётся на нажатом чипе) и с
      перепроверкой ожиданий `appliedCaption` из задач 9–10: они ждут
      появления НОВОЙ подписи, а с прежним кадром в DOM ожидание сменится.
- [ ] `just ci`.
