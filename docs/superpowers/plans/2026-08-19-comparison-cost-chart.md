...
> **Исправлено по ревью задачи 5.** Здесь стояло «меняется только
> `_median_dict`: он получает ставку показа и режим», и это разошлось с
> реализацией: `_median_dict` зовётся из ДВУХ мест, и правка внутри неё
> протащила бы `shown_per_sqm` в медианы всех 253 строк дерева, чего контракт
> спеки не вводит (§2.10 называет только `totals_medians`). Задача 5 завела
> вместо этого ОТДЕЛЬНЫЙ сериализатор. Формулировка оставалась опасной не
> абстрактно: задача 6 добавляет то же правило к `nominal`, и исполнитель,
> читающий план сверху вниз, получил бы указание положить его в `_median_dict`.

Правило присутствия живёт **в одном месте** — в `_totals_median_dict`
(заводится задачей 5, зовётся ТОЛЬКО на точке сборки `totals_medians`).
`_median_dict` остаётся как есть и продолжает обслуживать `rows[].medians`.
Ни клиент, ни `_row_cells` о правиле не знают: клиент рисует линию, если поле
пришло и не `null` (плюс проверяет единицу диаграммы, которую поле не
описывает).
...
**Ставка, которую получает сериализатор, — `effective_single_rate`**, то есть
после подстановки предвыбора, а не параметр запроса. С параметром запроса режим
«Единая» без явной ставки в адресе отдал бы медиану `null` при столбцах с
числами (devlog §3.4).

**Задача 6 обязана ПЕРЕИСПОЛЬЗОВАТЬ это правило, а не повторить его.** Спека
требует того же правила присутствия у `nominal.shown_per_sqm` (§2.8, §2.10), и
два независимых `if vat_mode == …` однажды разойдутся. Либо `nominal` собирается
внутри того же `_totals_median_dict`, либо выносится предикат вида
`_shown_axis_defined(vat_mode)`, и оба места читают его.
...
- [x] Отказ 400 для `ids` вместе с `q`; отдельным тестом.
- [x] То же для `object_id`; отдельным тестом.
- [x] То же для `contractor_id`; отдельным тестом.
      Все три проверены снятием защиты: 3 failed, 14 passed, ничего лишнего
      (devlog §5.1).
- [x] Сужение по классу в форме `ids`.
- [x] Сужение по классу в форме `all=1`.
      Из этой ветви убрана передача `rate_class_id` в `filtered_contract_ids` —
      иначе сужение применялось бы дважды (devlog §3.2).
- [x] Тест равенства форм на одинаковом множестве при многозначном классе
      (DoD 2) — поверх существующего
      `test_filter_form_selects_the_same_ids_as_the_contracts_list`.
- [x] Тест: класс, которого нет в выборке, даёт ПУСТУЮ выборку, не ошибку
      (DoD 4).
- [x] Тест: `ids` с `all=1` по-прежнему 400 — не потерять существующее.
      `test_both_forms_at_once_is_a_request_error` не правился и остался зелёным,
      в том числе при снятой новой защите.
- [x] `just ci`.

Сверх списка: шаг сужения поставлен под `if rate_class_id is not None` (замер
бюджета запросов — devlog §2.3), и заведён тест на то, что отказ 400 при пустом
списке классов не исчезает на пустой выборке — проверен снятием (devlog §5.2).
...
- [x] `Selection` и правка `resolve_selection`.
- [x] `build_comparison(…, facet_ids=None)` — сигнатура списка сохранена.
      Замер подтверждён: 61 вызов в тех же файлах, оба генератора в `docs/`
      не тронуты (devlog §2.2).
- [x] `analytics.py:195` и `reports.py:173` распаковывают `Selection`.
- [x] Существующие тесты `test_comparison_selection.py`, читающие возврат
...
- [x] Тест: `build_comparison` без `facet_ids` даёт facet по самой выборке.
- [x] Фикстура на два договора одного класса и один другого.
- [x] `available_rate_classes` в ответе; порядок по `title`.
      Ключ сортировки доопределён до пары `(title, id)` — по одному `title`
      порядок тёзок зависел бы от выборки (devlog §4.3).
- [x] `rate_class_id` в колонке; значение — снимок из договора.
- [x] Тест: facet присутствует БЕЗ сужения (DoD 7).
- [x] Тест: при `rate_class_id=<один>` facet несёт ВСЕ классы выборки —
      негативный, снятие обязано ронять (DoD 8).
      Снято, роняет 3 теста из 30, DoD 10 при этом НЕ падает (devlog §5.3).
- [x] Тест: facet после остальных фильтров — при `all=1&contractor_id=N` в нём
      только классы этого подрядчика (DoD 9).
- [x] Тест: `count` равен числу колонок при сужении ровно до этого класса
      (DoD 10).
- [x] Тест: переклассификация объекта не меняет `rate_class_id` колонки
      (DoD 12).
- [x] Обновить эталон задачи 1, привести дифф целиком.
      20 вставок, 0 удалений в ответе; лист сохранил ТОТ ЖЕ `sha256`
      (devlog §1, «Приёмка диффа задачи 4»).
- [x] `just ci`.

Сверх списка: facet посчитан НОЛЬ дополнительными запросами (devlog §2.4);
надмножество `facet_ids` проверяется `RuntimeError`-ом, а не соглашением
(devlog §4.2, §5.4); исправлен `test_ids_and_filter_selection_agree`, ставший
неверным от ревизии §2.6, и рядом закреплена старая пара адресов в новом
прочтении (devlog §3.2а).
...
- [x] `_totals_median_dict`; вызов только в точке сборки `totals_medians`.
      Получает `effective_single_rate`, а не параметр запроса (devlog §3.4).
- [x] Тест: `rows[].medians` ключа `shown_per_sqm` НЕ несут — негативный,
      снятие обязано ронять. Снято, роняет ровно его (devlog §5.6).
- [x] Тест: при `net` `shown_per_sqm == value`.
- [x] Тест: при `single` `shown_per_sqm == net_to_gross(value, rate)` —
      утверждением о числе (DoD 22а). Числа выведены из фикстуры, а не из
      плана: медиана нетто 200 при ставке 20 % даёт 240. Число стенда
      (129 800 → 155 760) проверяется на задаче 14.
- [x] Тест: при `own` ключа нет — негативный, `assert "shown_per_sqm" not in …`
      (DoD 22б). Снято, роняет целевой тест И эталон (devlog §5.5).
- [x] Тест: при менее чем трёх сопоставимых ключ ПРИСУТСТВУЕТ и равен `None` в
      `net`/`single` (DoD 24).
- [x] `just ci` — все шаги, кроме `test-frontend`: он недостижим по внешней
      причине (devlog §4.3б). Фронтенд этой задачей не тронут.

Сверх списка: тест на режим `single` БЕЗ явной ставки — медиана обязана прийти
по предвыбранной ставке, а не `null` (devlog §3.4).
...
- [x] `apply_adjustment` извлечён из `load_rollups`; `load_rollups(adjustment=…)`
      делегирует, место умножения одно.
- [x] `RuntimeError` на смету без коэффициента сохранён и покрыт тестом —
      **снятие проверки обязано ронять тест** (защита от смешивания
      приведённых с неприведёнными). Снято: 2 failed (оба слоя), devlog §5.8.
- [x] Разделение роллапов на номинальные и приведённые из одного чтения.
      Справочник статей читается ОДИН раз и передаётся в оба места — иначе
      бюджет запросов закрытой фичи вырос бы с 7 до 8 (devlog §2.5).
- [x] `nominal` в ячейках `totals` по трём корзинам.
- [x] `nominal` у `totals_medians` — с тем же правилом присутствия
      `shown_per_sqm`, что в задаче 5. Правило ПЕРЕИСПОЛЬЗОВАНО через предикат
      `_shown_per_sqm_axis_defined`, второй копии условия про режимы нет.
- [x] Тест: без приведения ни одного ключа `nominal` — негативный (DoD 13).
      Снято: 2 failed из 1130, включая эталон (devlog §5.9).
- [x] Тест: `nominal.shown_per_sqm × коэффициент == shown_per_sqm` там, где
      коэффициент колонки определён (DoD 14). Плюс форма ячейки закреплена
      явно, и ожидание считается `adjust_amount`, а не произведением.
- [x] Тест: при РАЗНЫХ коэффициентах ДГП и ДС номинал «Итого» верен —
      на `contract_with_amendment_dates` (DoD 15).
- [x] Тест: `totals_medians[bucket].nominal` равна медиане ТОГО ЖЕ запроса без
      ряда — сравнением двух ответов, а не пересчётом (DoD 16).
- [x] Тест: `rows` номинала не несут — утверждением об отсутствии ключа
      (DoD 17). Снято: 1 failed из 1130 (devlog §5.10).
- [x] Обновить эталон задачи 1: диффа быть НЕ должно (без приведения ответ не
      меняется). **Диффа нет** — devlog §1, «Приёмка диффа задачи 6».
- [x] `just ci` — все шаги, кроме `test-frontend` (недостижим по внешней
      причине, devlog §4.3б). Фронтенд этой задачей не тронут.

Правило приёмки диффа эталона (задача 1) закрыто целиком: обновления пришлись
ровно на задачи 4 и 6, и оба диффа состояли ровно из объявленной дельты.
...
- [ ] **Число стенда из DoD 22а:** медиана нетто 129 800 ₽/м² при ставке 20 %
      даёт линию на 155 760, и отношение «столбец к медиане» совпадает с
      режимом «Без НДС». В интеграционный тест это число не попало намеренно —
      там своя фикстура (медиана 200 → 240), поэтому именно стендовое число
      проверяется здесь, иначе выпадает из приёмки вовсе (заведено ревью
      задачи 5).
