...
- [ ] Прогнать `test_comparison_baseline.py` ДО единой правки кода: должен быть
      зелёным. Красный здесь означает, что ветка уже что-то сломала.
- [ ] Записать в devlog хеш эталона до правок.
...
- [ ] Импорт `Sequence` в `crud/contracts.py` — его там сейчас нет
      (`DomainError` уже есть, 13 употреблений).
- [ ] Правка функции.
- [ ] Тест: одиночный `rate_class_id` даёт то же множество, что до правки.
- [ ] Тест: список из двух классов даёт объединение.
- [ ] Тест: пустой список — 400 (решение плана 2).
- [ ] `just ci`.
