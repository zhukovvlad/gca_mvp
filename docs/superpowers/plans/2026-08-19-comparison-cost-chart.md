...
`addDecimalStrings` — `src/lib/decimal.ts`. **Поправка задачи 11:** здесь
стояло ещё и `compareDecimalStrings` в том же модуле — записи НЕТ, функция
продублирована приватно в `StructureRing.tsx` и `UnallocatedPanel.tsx`. То есть
инвентарь «проверено grep-ом» содержал одну ложную строку; задача 11 её не
использовала, но следующий исполнитель пошёл бы за ней (devlog §4.7).
...
1. **`rate_class_id` разбирается на границе адреса, а не в домене.** Строка
   `2,3` — это формат URL; `resolve_selection` принимает `Sequence[int]`, и слой
   домена про запятые не знает.

   > **Формулировка исправлена по ревью задачи 7.** Здесь стояло «разбирается в
   > РОУТЕРЕ, а не в CRUD», и это противоречило задаче 7, которая требует
   > положить хелпер рядом с `parse_ids_param` — а тот живёт в
   > `crud/comparison.py`. Смысл решения не в файле, а в том, ЧТО именно знает
   > домен: `resolve_selection` получает уже список чисел. Физически хелпер
   > обязан лежать в общем модуле, потому что маршрутов сравнения два, и второй
   > разбор одного параметра на этом проекте уже расходился.
...
- [x] Обновлять эталон **только** в задачах 4 и 6, и в каждом коммите приводить
      дифф эталона целиком: он обязан состоять ровно из объявленной дельты.
      Любая лишняя строка — дефект, а не «эталон устарел».
      Закрыто: обновления пришлись ровно на задачи 4 и 6, оба диффа состояли из
      объявленной дельты. Правило приёмки при этом ДОПОЛНЕНО — дифф проверяется
      и по составу, и по ЗНАЧЕНИЯМ: сквозное число в снимке дефект сам по себе
      (devlog §4.3а).
...
- [x] Хелпер разбора рядом с `parse_ids_param`, один на оба маршрута —
      `parse_rate_class_id_param`.
- [x] Подключён в `get_comparison`.
- [x] Подключён в `comparison_report`.
- [x] Тест: `?ids=…&rate_class_id=2,3` — 200 и меньше колонок (DoD 1).
- [x] Тест: одиночный `rate_class_id=2` — прежняя семантика (DoD 5).
- [x] Тест: `?ids=…&q=…` — 400 (DoD 3), на уровне HTTP.
      **Тестов стало ТРИ, а не один:** спека требует по каждому из трёх
      параметров отдельным тестом, и здесь чек-лист был слабее спеки.
- [x] Тест: лист XLSX на тех же параметрах — по той же выборке (DoD 6).
- [x] `just ci` — все шаги, кроме `test-frontend` (недостижим по внешней
      причине, devlog §4.3б).

Сверх списка: пустой `rate_class_id=` — 400 из ЕДИНСТВЕННОГО места, guard'а
задачи 2, а не второй проверкой в хелпере (проверено снятием); нечисловой
элемент — 400, и различитель отказа ужесточён, потому что моя же страховка из
предыдущего коммита маскировала эту защиту (devlog §5.11); сторож паритета двух
маршрутов расширен на новый параметр (devlog §5.12); освежены докстроки обоих
маршрутов — после ревизии §2.6 они утверждали, что `rate_class_id` входит в
четвёрку фильтров формы `all=1`.

Границы соблюдены: `/matrix`, `/bank-comparison` и экран списка договоров
не тронуты (Global Constraint 7).
...
- [x] Типы. Заведён `ComparisonRateClassFacet` и пять полей; формы вложенного
      `nominal` у ячейки и у медианы РАЗНЫЕ и не унифицированы.
- [x] `ComparisonParams.rate_class_id` **уже существует** как `string`
      (`domain.ts:1283`) — поле не заводится, расширяется его СЕМАНТИКА до
      строки со списком; правка идёт в докстроку, а не в объявление.
- [x] Фикстуры и MSW-обработчики с новыми полями.
      `handlers.ts` был БЛОКЕРОМ, найденным ревью: довод «поля протекают через
      фикстуру автоматически» верен только для `available_rate_classes`, а мок
      отражает `vat_mode` эхом и собирает приведённый ответ отдельной функцией —
      `shown_per_sqm` и `nominal` пришлось кладть руками. Задачи 11–12 иначе
      получили бы ЗЕЛЁНЫЕ тесты на отсутствующую фичу (devlog §3.7).
- [x] `just ci` (здесь важен `tsc`).
...
- [x] Эффект записи `single_rate`. Отдельного условия на `vatMode` в нём НЕТ:
      несёт клиентская проводка `params`, и она закреплена отдельным тестом, а
      не чтением (devlog §3.6а).
- [x] Тест: открытие `?vat_mode=single` без ставки дописывает её в адрес
      (DoD 33) — утверждением о ЗНАЧЕНИИ, а не о появлении параметра.
- [~] Тест: сужение по классу не меняет действующую ставку — негативный,
      снятие эффекта обязано ронять (DoD 34). **РАЗДЕЛЁН.** Адресная половина
      («после записи предвыбор в игру не входит») закрыта здесь тестом, где
      действующая ставка и предвыбор РАЗВЕДЕНЫ — на фикстуре они совпадают, и
      тест на их совпадении не различает, какое поле читает экран. Переход
      через сужение перенесён в задачу 10: единственный контрол сужения —
      чипы, и без них тест был бы вакуозным (devlog §3.6б).
- [x] Сверх списка: тест на то, что вне «Единой» клиент ставку серверу не
      отправляет — пин на проводку, из которой у эффекта нет условия на режим.
- [x] `just ci`.

**DoD:** 33, 34 (адресная половина; переход — задача 10).
...
- [x] Группа чипов, порядок из ответа. `fieldset`/`legend` даёт роль группы
      сам — второй `role="group"` с тем же именем завёл бы два узла в дереве
      доступности.
- [x] Запись в адрес: id по возрастанию, при полном наборе параметра нет.
      Полнота мерится по `available_rate_classes`, а он от сужения не зависит.
- [x] Снятие последнего класса не срабатывает (DoD 30). Выбранное СЕЧЁТСЯ с
      фасетом: без сечения `rate_class_id=1,9` считался бы двумя выбранными и
      защита обходилась бы — замерено, devlog §5.16. Молчание контрола
      объяснено `aria-disabled` с подсказкой, а не `disabled`: выключенная
      кнопка спрятала бы защиту за DOM.
- [x] Тест: чипы видны и после перезагрузки с сужением (DoD 8 на клиенте).
- [x] **Тест ПЕРЕХОДА из задачи 9 (DoD 34):** клик по чипу при уже записанной
      ставке её не меняет — негативный, снятие записи обязано ронять. Приехал
      сюда потому, что чип и есть тот контрол сужения, которого задаче 9 не
      хватало; адресную половину DoD 34 задача 9 уже закрыла. Мок обязан делать
      предвыбор ЗАВИСЯЩИМ от сужения — иначе снятие записи ничего не меняет и
      тест вакуозен (это предпосылка, а не деталь).
      Предпосылка мерится ПОДПИСЬЮ ОТВЕТА, а не адресом: через адрес она
      проверялась бы тем самым эффектом, снятие которого обязан ловить сам тест
      (devlog §5.17).
- [x] `just ci`.

**DoD:** 30, 31, 34 (переход).

Сверх списка: заведён тест на снятие последнего ВИДИМОГО класса при фантомном
id в адресе (devlog §5.16); чип несёт `count` фасета, и это закреплено
утверждением — без него подмена `count` на `id` оставила бы набор зелёным.

**Отложено в задачу 12 с объяснением:** каждый клик по чипу меняет ключ запроса,
и на время загрузки вся панель вместе с чипами заменяется скелетоном — фокус
клавиатуры уезжает в `<body>`. Поведение дофичевое (`updateVatMode` делает то
же), но на многозначном фильтре начинает мешать: два переключения — два
исчезновения. Лечится `placeholderData: keepPreviousData` в `useComparison`,
что меняет состояния загрузки ВСЕГО экрана и требует своих тестов, — поэтому
владелец задача 12, а не эта. Найдено ревью задачи 10.
...
- [x] `buildCostChartBars` и `costChartAxisTop` — чистые, без запросов.
      `costChartAxisTop` принимает УЖЕ ПОСЧИТАННЫЕ бары, а не `Comparison`:
      иначе «пул это то, что нарисовано» держалось бы на дисциплине вызывающего
      (правка по ревью, devlog §4.8).
- [x] `costChartData.test.ts`: порядок столбцов (DoD 18); геометрия обоих знаков
      ...
      Сверх списка: призрак снижения ОДИН поднимает верх оси — без этого набора
      его участие в пуле не сторожилось ничем (devlog §5.18).
- [x] Компонент, порядок столбцов от старых к новым (DoD 18).
- [x] Симметричное изображение поправки (DoD 19, 20, 21).
- [x] Медиана по правилу спеки диаграммы стоимости §2.4 (DoD 22, 22а, 22в) и её
      отсутствие с объяснением (DoD 22г, 24). Причин отсутствия ТРИ, и у каждой
      своя подпись: ось сумм; поля нет вовсе («своя ставка»); поле есть и
      `null` («сопоставимых меньше трёх»).
- [x] Единица диаграммы, переключатель у самой диаграммы (DoD 26).
- [x] Следование корзине (DoD 27).
- [x] Ось от нуля и по применённому состоянию (DoD 28, 28а).
- [x] Прокрутка и минимальная ширина столбца (DoD 29). Плюс СЕТКА по засечкам
      внутри прокрутки: жёлоб вынесен наружу, и без линий уехавшему вправо
      столбцу сличать высоту не с чем (найдено ревью).
- [x] Договор без суммы занимает место с причиной (DoD 25).
- [x] `ContractCostChart.test.tsx`: линия медианы есть в `net` и `single`, нет в
...
- [x] `ContractCostChart.test.tsx`: **`x` и `width` обеих полос совпадают** —
      иначе штриховка уезжает сбоку, и это единственная проверка, которая ловит
      потерянный `barGap`. Проверено снятием.
- [x] `ContractCostChart.test.tsx`: договор без суммы остаётся в ряду, с причиной
      из `incomplete_reasons`, а не выпадает (DoD 25).
- [x] `ContractCostChart.test.tsx`: на оси сумм плашка отклонения подписана
      «к медиане ₽/м²» — **негативный, снятие подписи обязано ронять тест**
      (DoD 26). Проверено снятием.
- [x] Сверх списка, по ревью: закрыты ПЯТЬ швов отрисовки, где снятие не роняло
      ничего — линии медианы и риски номинала на полотне, шкала жёлоба и число в
      его плашке, подпись значения у верха сплошной части, штриховка промежутка
      (devlog §5.19).
- [x] `just ci`.
...
- [x] Приведение по умолчанию выключено: без инфляционных параметров в адресе
      ...
      Тестов ДВА, а не один: половина про номинальную линию медианы в «Своей
      ставке» вакуозна по построению (линии там нет вовсе), и ей нужен режим
      «Без НДС» с измеренной предпосылкой (devlog §5.20).
- [x] Таблица сравнения НЕ получает колонку «Медиана» — утверждением об
      отсутствии (DoD 22в, Global Constraint 6). Утверждение различает ТАБЛИЦУ и
      диаграмму: в жёлобе диаграммы слово «медиана» стоит законно.
- [x] Диаграмма не ломается на выборке без сумм.
- [x] Диаграмма не ломается при менее чем трёх сопоставимых. Плюс отсутствие
      ПЛАШЕК отклонений, которого требует DoD 24 наравне с отсутствием линии:
      первая редакция фикстуры оставляла живые проценты при мёртвой медиане —
      состояние, которого сервер не выдаёт (devlog §5.21).
- [x] **Пришло из задачи 11 (DoD 28а, вторая половина):** ВЫБОР РЯДА ось не
      двигает — числа и адрес меняет только «Привести». Задача 11 закрыла лишь
      «ось по применённому состоянию»; «выбор сам по себе ничего не меняет» —
      свойство экрана, а не чистой функции, и до сих пор не значилось ни в одном
      чек-листе (найдено ревью задачи 11). Сравниваются засечки жёлоба до и
      после выбора, плюс счётчик запросов приведения.
- [~] **Пришло из задачи 10:** клик по чипу класса не должен ронять фокус
      клавиатуры и гасить всю панель скелетоном. **Дефект ЗАМЕРЕН, прописанное
      лечение ОТКЛОНЕНО замером, отложено в долг.** `placeholderData:
      keepPreviousData` постоянно ломает три существующих утверждения об адресе
      (DoD 18 спеки сравнения, «выход из «Единой»», сброс приведения к
      номиналу): на кадре плейсхолдера эффекты, пишущие ответ сервера в адрес,
      читают прошлый ответ и возвращают устаревшее значение, а откатить свою же
      запись им нечем. Ломает не тайминг, а состояние страницы — то есть
      «принципиальное», при котором задача обязана остановиться. Подтверждено
      дважды независимо, оба раза ровно три падения из 552. Запись 16
      `docs/TECH_DEBT.md` — там же второй путь лечения, не требующий правки
      запроса. Теста нет намеренно: тест на несуществующее поведение был бы
      либо вечно красным, либо неправдой.
- [x] `just ci`.

**DoD:** 32.

Сверх списка: предпосылка эффекта записи ставки («ответ описывает ТЕКУЩИЙ
адрес») названа в его докстроке — она не выражена типом и держится на настройке
запроса, а сторожа у неё те самые три теста.

---

## Task 13: `AGENTS.md` → v6.11

- [x] §7 п. 7: диаграмма итога и фильтр по классу объекта, оба выражены в URL.
      Плюс названы: порядок столбцов, противоположный таблице, и почему это цена,
      а не дефект; единица диаграммы, которой в URL НЕТ; запись действующей
      ставки показа в адрес.
- [x] §11: грабли `Decimal.//` — усечение к нулю, «потолок» через `-(-a // b)`
      занижается. Утверждение проверено прогоном, а не памятью: для `Decimal`
      выражение даёт 4 интервала, для `int` — 5.
- [x] Врезка версии с обоснованием: ревизия §2.6 спеки сравнения и коррекция
      200 → 400. Сказано прямо, что осознанной ревизии ИНВАРИАНТА в фиче нет —
      ревизия уровнем ниже, — и отдельно, что медиана впервые показана числом.
- [x] Сверх списка: §11 пополнен второй граблей — случайный `npx` из каталога без
      инструмента уносит `node_modules/.bin` соседнего проекта, после чего `npx`
      в ПРАВИЛЬНОМ каталоге молча берёт установку из реестра. Из того же
      семейства, что две уже стоящие там про `npx`; стоило получаса на этой
      фиче (devlog §4.11).
...
- [x] Прогон всех состояний макета на живом стенде, включая цель раньше сметы.
      Вёлся системным Chrome под Playwright; devlog §6.2. Нашёл ТРИ дефекта,
      которых не видели 553 теста (devlog §4.12).
      **Прогон пришлось делать ВТОРОЙ раз, и второй нашёл дороже первого**
      (devlog §4.13): первый читал числа и атрибуты, а на экран не смотрел — все
      пять CSS-переменных диаграммы не существовали, столбцы были чёрными,
      штриховка, риска и обе линии медианы не рисовались вовсе при верной
      геометрии. Нашёл пользователь по снимку. Оттуда же четыре расхождения с
      макетом: не было легенды, формат чисел не следовал `fmt_value`, плашка
      отклонения не называла «к медиане», не было дубля медианы текстом.
- [x] **Обещания, отложенные на стенд ИМЕННО этой задачей — перечислены, чтобы
      не испарились** (найдено ревью задачи 11; §12, `replaying-new-rules`,
      слой 3): фактическая прокрутка полотна (`scrollWidth` против
      `clientWidth`; замер спеки — 542 против 500); полоса подписей совпадает со
      столбцами (jsdom раскладку не считает, тест сторожит только объявленные
      ширины и отсутствие зазора); подпись значения стоит у верха сплошной части
      на СНИЖЕНИИ; линии сетки видны у столбца, уехавшего вправо; приведённые
      значения СДП-1-МР и 12-СИТ-МР по «Фактической инфляции» на 08.2026 не
      превышают 200 000 — иначе верх оси из DoD 28а другой.
- [x] **Число стенда из DoD 22а:** медиана нетто 129 800 ₽/м² при ставке 20 %
      ...
      задачи 5).
- [x] `docs/devlog/2026-08-19-comparison-cost-chart.md`: что сделано, замеры,
      отступления от плана, найденные грабли. Плюс §7 — соответствие
      «требование спеки → тест» списком, и §8 — что фича изменила в правилах.
- [x] Если фича выстрадала правило работы — инсайт и строка в §12. Заведён
      `docs/insights/unobservable-in-the-runner.md` (утверждение, которого прогон
      не может опровергнуть) и дописаны слои 11–12 в `verifying-guards.md`.
- [x] **Прогон пришлось делать ТРЕТИЙ раз** (devlog §4.14, §4.15). Второй нашёл
      дефекты в том, что я трогал; третий — в том, чего не трогал: панель
      управления над диаграммой расходилась с тем же согласованным макетом на
      восьми пунктах (подписи слева вместо «над», светло-зелёное «нажато» вместо
      тёмного, нет разделителей сегментов, `opacity-50` вместо подложки у
      выключенного, нет видимой подписи у единой ставки, не выделена группа
      поправки, порядок блоков обратный, нет карточки-обёртки). Нашёл
      пользователь по снимку. Оттуда же два дефекта тёмной темы, жившие всю
      фичу: `dark:`-утилиты shadcn перебивают токен-классы специфичностью.
      Оформление вынесено в `controlStyles.ts`, четыре теста на порядок и
      вложенность проверены снятием (devlog §5.25).
- [x] **Прогон ЧЕТВЁРТЫЙ и ПЯТЫЙ** (devlog §4.16, §4.17). Четвёртый: у диаграммы
      не было подсказки при наведении (`ChartTooltip` лежал в `chart.tsx`
      неимпортированным), в подписи столбца не было даты подписания, а столбец был
      40px против макетных 78px — 16 % слота вместо 85 %. Закрыто; четыре
      паспортных поля протащены в `CostChartBar` из `ComparisonColumn`, сервер не
      правился. Пятый: `activeBar`, которым я повторил подсветку наведения из
      макета, необратимо съедал штриховку промежутка у наведённого столбца —
      проп снят, замер держит все восемь прямоугольников в трёх состояниях.
      Попутно заведена запись 17 `docs/TECH_DEBT.md` (секция диаграммы выпирает
      за окно 560px на 30px — дефект дофичевый, проверено возвратом из `HEAD`).
- [x] **Внешнее ревью PR (Codex) нашло две неправды диаграммы** (devlog §4.18):
      отсутствие ТЭП выдавалось за отсутствие суммы на оси ₽/м² (и это же
      озвучивалось скринридеру), а подсказка объявляла «Приведено, рост» при
      разных множителях смет, не проверяя направление. Закрыто различителем
      `perSqmBlockedByArea` в данных, словом из общего словаря
      (`AREA_MISSING_LABEL`) и подсчётом направления по величинам с третьим
      состоянием для равенства. Двенадцать тестов, шесть снятий, включая
      прибивание флага к `true` (devlog §5.27). Заодно исправлена МОЯ
      недостижимая фикстура — второй такой случай за фичу.
- [x] **Второй круг внешнего ревью** (devlog §4.19): первое замечание закрыто
      полностью, у второго остался хвост в той же строке — разность считалась
      склейкой `-${nominal}`, и на отрицательном номинале направление терялось в
      обе стороны. Заведён `compareDecimalStrings`, восемь тестов, три снятия,
      включая сравнение по модулю (devlog §5.28).
- [x] **Третий круг внешнего ревью** (devlog §4.20): тон чипа поправки красил
      множитель ровно 1 как рост — нейтральный текст «поправка 0,0%» выходил
      жёлтым, тогда как подсказка на тех же данных уже отвечала нейтрально.
      Прошлым кругом я назвал это «дофичевым» и отложил; ревьюер поправил по
      факту — компонент заводится этим же PR. Закрыто третьим состоянием
      направления (`"up" | "down" | "flat"`) и картой тонов; нейтральный тон взят
      тот же, что у чипа «разные», — новый цвет не понадобился. Пять тестов,
      четыре снятия (devlog §5.29).
- [x] PR со ссылками на спеку и план — [#30](https://github.com/zhukovvlad/gca_mvp/pull/30).
- [x] **Последнее расхождение с макетом закрыто перед мержем** (devlog §4.21):
      корзины шли «ДГП ДС Итого» против макетных «Итого ДГП ДС», и нажатая по
      умолчанию «Итого» оказывалась третьей. Три строки в `labels.ts` плюс тест на
      видимый порядок.
