# Ф7: ручной разнос разделов сметы по статьям — план реализации
...
```python
#: Второй источник привязки (Ф7). Порядок утверждений — спека Ф7 §2.1:
...
```python
"""Схема решений о статье (спека Ф7 §2.2, §4.2). Integration: нужен живой PG."""
...
Фаза 7, Ф7. Решение о статье раздела хранится отдельной таблицей и применяется
...
class EstimateCategoryOverride(Base):
    """Ручное решение о статье строки-раздела (Ф7, миграция 0011).
...
```python
"""Применение решений о статье (спека Ф7 §2.3–2.5, §4.3)."""
from __future__ import annotations
...
```python
"""Применение ручных решений о статье раздела (спека Ф7 §2.3–2.5).
...
```python
"""HTTP-слой разноса (спека Ф7 §2.7, §4.3)."""
from __future__ import annotations
...
```python
"""Роутер ручного разноса разделов сметы по статьям (спека Ф7 §2.7).
...
```python
    # Решения о статьях уходят каскадом вместе со сметой (спека Ф7 §1.8), и
    ...
    # перечень: истории решений проект не ведёт (спека Ф7 §3.3).
    lost = db.execute(
        sa.select(
            sa.func.count(),
...
```typescript
/** Раздел сметы без статьи — узел ДЕРЕВА разносимого (спека Ф7 §2.6). */
...
По §7 спеки: `phase7-frame.md` — снять пункт 4 «вне скоупа v1» с объяснением
причины, добавить строку Ф7 (миграция 0011, ветка `feat/unallocated-override`);
`AGENTS.md` §3 — ручной разнос в правах `member`; §7.4 — признак ручного разноса и
печатная сноска; §10 DoD — формулировка **без** глобального обещания нуля, с
перечислением трёх границ §5.5 и отдельным пунктом про смету стенда.
...
git commit -m "docs(Ф7): devlog, правки рамки фазы 7 и AGENTS.md"
