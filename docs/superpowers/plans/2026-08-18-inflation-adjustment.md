...
> **ПОПРАВКА 2026-08-19 (по итогам внешнего ревью реализации). Бюджет с
> приведением — СЕМЬ, а не восемь. Это уточнение ПЕРЕКРЫВАЕТ все три места плана,
> где сказано «восемь»** (здесь, «Решения плана» п. 4 и задача 8 шаг 6): текст тех
> абзацев оставлен как запись принятого тогда счёта и задним числом не
> переписывается, но действующее число — семь.
>
> Восьмым был отдельный `SELECT` строки ряда. Он убран потому, что ряд и его годы,
> прочитанные ДВУМЯ операторами, в `READ COMMITTED` могут дать смешанное
> наблюдение: старые `series_name`/`series_note`/`series_updated_at` рядом с уже
> новыми коэффициентами, если параллельный `PATCH` закоммитился между чтениями.
> Лист печатает и то и другое рядом, а `series_updated_at` — единственный носитель
> компромисса §2.10 («ссылка не гарантирует исторического результата»), поэтому
> смешанное наблюдение соврало бы о том, какая редакция ряда дала числа. Атомарная
> ЗАПИСЬ задачи 5 этого не закрывает — она запрещает «половину ряда записана», но
> не обещает, что два независимых `SELECT` увидят одно состояние.
>
> Ряд читается вместе с годами одним `LEFT JOIN`. Выбран он, а не `FOR SHARE`,
> который предлагало ревью: один оператор делает смешанное наблюдение
> **непредставимым**, тогда как блокировка лишь предотвращает его
> договорённостью двух модулей о том, кто какой замок берёт. Побочно ушёл `db.get`,
> отдающий объект из identity map БЕЗ запроса, — с ним бюджет зависел бы от того,
> грелась сессия или нет, то есть бюджетный тест мерил бы не то, что в проде.
>
> Опасность ЗАМЕРЕНА, а не изложена:
> `test_two_separate_selects_can_observe_a_half_applied_edit` показывает «Ряд до
> правки» рядом с коэффициентом `1.500`. Коммит — `dfdb7d9`; в devlog (задача 16)
> это отступление входит отдельной строкой.
...
4. **Три дополнительных запроса при включённом приведении** — `resolve_inflation`
   ...
   > **Уточнение 2026-08-19:** дополнительных запросов ДВА, полный бюджет — **5 и
   > 7**. Ряд читается вместе со своими годами одним оператором; обоснование и
   > замер — в поправке к разделу «Архитектура», она перекрывает это место.
...
- [x] **Step 1:** тесты (красные) по списку правил.
- [x] **Step 2:** реализация в указанной форме. Доменные проверки года
      (`coefficient > 0`, `require_text(source)`) живут в `_apply_year`, а НЕ в
      схеме запроса (Global Constraints).
- [x] **Step 3:** тест атомарности: `PATCH` с двумя годами, где ПЕРВЫЙ валиден
      ...
      **Почему именно пустой `source`, а не `coefficient = "-1"`:** отказ обязан
      ...
      > **Уточнение 2026-08-19 (внешнее ревью реализации).** Текст шага оставлен
      > как есть — это запись принятого тогда решения; здесь он уточняется.
      > Утверждение «`detail` — строка доменного отказа, а не список pydantic» на
      > уровне CRUD **вакуозно**: схема запроса в этом вызове не исполняется
      > вовсе, поэтому `isinstance(detail, str)` останется зелёным при любой
      > будущей схеме — то есть будет читаться как живая защита, будучи мёртвой
      > (§12, снятие защиты, слой 7). Из теста задачи 5 оно убрано, и настоящая
      > проверка перенесена в **задачу 6**: `PATCH` с пустым `source` обязан
      > вернуть `422` со СТРОКОВЫМ `detail`, а не список ошибок pydantic. Сама
      > атомарность закрыта здесь и подтверждена снятием (шаг 4).
- [x] **Step 4 — НЕ ДЕЛЕГИРУЕТСЯ, снятие защиты (DoD 26):** перенести
...
- [x] **Step 5 (DoD 37):** тест метки времени — **на `committing_db`**, не на
...
- [x] **Step 6:** добавить `inflation_index_values` и `inflation_series` в
...
- [x] **Step 1:** роутер по образцу `routers/rate_standards.py` (тот же приём:
      `require_admin` на изменяющих, чтение — всем аутентифицированным).
- [x] **Step 2:** регистрация в `main.py` рядом с нормативами, с комментарием о
      праве.
- [x] **Step 3:** тесты HTTP: `member` читает список и значения (200); `member`
      ...
      **отсутствует** в списке по умолчанию и **присутствует** при
      ...
      404.
- [x] **Step 4:** тест `float` на входе: `{"coefficient": 1.083}` (число, не
      строка) — `422`; `{"coefficient": "1.083"}` — принято; в ответе
      `coefficient` приходит **строкой** (прогон по конкретному входу, а не
      чтение кода).
- [x] **Step 5:** прогон файла.
- [x] **Step 6 (перенесено из задачи 5, шаг 3; уточнение 2026-08-19):** `PATCH`
      с пустым `source` через HTTP обязан вернуть `422` со **строковым** `detail`,
      а не список ошибок pydantic. Здесь проверка не вакуозна: схема исполняется,
      и уход доменного правила в неё роняет тест. На уровне CRUD это утверждение
      было недоказуемо.
...
- [x] **Step 1:** фикстуры: `series_with_years(db, name, {2024: "1.075", …})` и
...
- [x] **Step 2:** тесты (красные): покрытие считается объединением, а не
...
- [x] **Step 3:** реализация — один запрос `Estimate ⋈ Contract`, затем
      `required_years` на смету, объединение, проверка, `coefficient` на смету.
- [x] **Step 4:** тест DoD 8: ДС **без** `data_prepared_on_date` у договора,
      чей `signed_date` попадает в год, полностью покрытый рядом → `422`,
      код `amendment_date_missing`, `estimate_ids == [id ДС]`.
- [x] **Step 5 — НЕ ДЕЛЕГИРУЕТСЯ, снятие защиты (DoD 8):** заменить правило
      ...
      снова.
- [x] **Step 6:** тест DoD 10 — цель раньше самой поздней сметы: коэффициент
...
- [x] **Step 1:** тесты (красные) по числам ниже.
- [x] **Step 2:** умножение в `load_rollups`, план — из задачи 7. Умножается
      **каждая ветвь `frozen_branches` до разделения на `direct` и
      ...
      вместе).
- [x] **Step 3:** громкий `RuntimeError` на смете без коэффициента при непустом
      `adjustment` (см. «Архитектура»), тест с урезанным `adjustment`.
- [x] **Step 4:** `_mode_caption` получает `InflationPlan | None` и дописывает
...
- [x] **Step 5:** `columns[].inflation_coefficient` и `inflation_factors` по
      правилу решения плана №1.
- [x] **Step 6:** абсолютный бюджет запросов: **5 без приведения, 8 с ним**
      (`_count_queries` уже есть — `test_comparison_buckets.py:101`). Существующий
      тест «не растёт с выборкой» остаётся — он про другое.

      > **Уточнение 2026-08-19:** в тесте стоит **5 и 7**. Обоснование — поправка к
      > разделу «Архитектура», она перекрывает это место; сам шаг выполнен именно
      > так, как требовал: абсолютным числом, а не «не растёт».
- [x] **Step 7 — НЕ ДЕЛЕГИРУЕТСЯ, снятие защиты (DoD 31):** зашить название ряда
...
- [x] **Step 1:** оба роутера принимают два одинаковых параметра; разбор
      `YYYY-MM` — `parse_target_month`, неверный формат → `400`.
- [x] **Step 2:** неизвестный ряд → `404`; архивный по явному id → **200 и
      приведение считается** (DoD 20, два разных утверждения).
- [x] **Step 3:** тест DoD 11 — выгрузка при отказе отвечает тем же
      структурированным `422`, `Content-Type` НЕ `…spreadsheetml…`, вложения нет
      (проверяется отсутствием вложения, а не содержимым листа).
- [x] **Step 4:** тест DoD 23 — экран и лист на ОДНОМ входе дают идентичный
      объект `detail`.
- [x] **Step 5:** тест DoD 18 — «текущий месяц» разрешается в названной зоне:
...
- [x] **Step 1:** при `data.get("inflation")` под подписью состава печатаются:
...
- [x] **Step 2:** тест DoD 13: все пять фактов года на листе присутствуют.
- [x] **Step 3:** тест DoD 32 (половина про лист): `source` года **есть на
...
- [x] **Step 1:** `coefficientLevel` + юнит-тесты (`1.083` → рост 8,3 %;
      `0.083` → снижение 91,7 %; `1.15` → рост 15,0 %; пусто → «—»; `abc` → «—»).
- [x] **Step 2:** диалог по образцу `ReapproveDialog.tsx`: `Dialog` + форма
...
- [x] **Step 3: режим СОЗДАНИЯ — тот же компонент, `series === null`.** Шлёт
...
- [x] **Step 4:** проп `missingYears?: number[]` — вход из баннера открывает окно
      с уже добавленными пустыми строками этих годов.
- [x] **Step 5:** архивный ряд: кнопка правки **выключена**, в шапке окна
      пояснение «сначала вернуть в активные».
- [x] **Step 6:** компонентный тест DoD 25: ввести `0.083` в поле коэффициента и
      увидеть «Снижение 91,7 %» **до сохранения**.
- [x] **Step 7 — НЕ ДЕЛЕГИРУЕТСЯ, снятие защиты (DoD 25):** удалить вызов
...
- [x] **Step 1:** третья вкладка рядом со «Ставки» и «Классы объектов»: таблица
...
- [x] **Step 2: архивация и возврат — две разные кнопки, по одному запросу
      ...
      архивного выключена.
- [x] **Step 3:** MSW-хендлеры рядов в `test/handlers.ts` + фикстура двух рядов
      и одного архивного в `test/fixtures.ts`.
- [x] **Step 4:** тесты: пустой справочник → создание ряда доводится до списка;
...
- [x] **Step 1:** `InflationControls` — `fieldset` с `legend` «Поправка на
      ...
      макету).
- [x] **Step 2:** запись параметров в URL — тем же приёмом, что `updateVatMode`
      (`setSearchParams(next, { replace: true })`). `Date.now()` на клиенте **не
      использовать**: месяц приходит от сервера.
- [x] **Step 3:** `InflationLevelsBar` — уровни по годам («2024 +7,5 % · 2025
...
- [x] **Step 4:** чип коэффициента в шапке колонки — **уровень** `+17,4 %`,
...
- [x] **Step 5:** `InflationRefusalBanner` — общий каркас, **два разных текста
...
- [x] **Step 6:** кнопки правки **не отрисованы** для `member`
      (`renderWithProviders(..., { initialUser: { role: "member" } })`) —
      проверяется отсутствием узла, а не его заблокированностью (DoD 35).
- [x] **Step 7:** тесты DoD 34: строка ряда, полоса и баннер рендерят **один
...
- [x] **Step 8:** тест DoD 36: после «Сохранить» сравнение перезапрашивается и
      показывает числа по новым коэффициентам (MSW отдаёт второй ответ).
- [x] **Step 9:** тест DoD 31 на экране: смена ряда меняет подпись оси И числа
      (два ряда с разными коэффициентами).
- [x] **Step 10:** `npx vitest run` и `npx tsc -b --noEmit` из `frontend/`.
...
- [x] **Step 1:** врезка v6.10 первой в блоке цитат, со ссылками на спеку и
      devlog. Changelog обязан назвать именно **добавление инварианта**, а не
      редакционную правку.
- [x] **Step 2:** §7 п. 3 — на экране нормативов появляется ряд индексов
      инфляции.
- [x] **Step 3:** §7 п. 7 — в URL страницы сравнения, кроме режима НДС и ставки,
      живут выбранный ряд и целевой месяц.
- [x] **Step 4:** §10 — новый совместимый инвариант: много-договорные
...
- [x] **Step 1:** `just ci` целиком, шаги **по отдельности**; `$LASTEXITCODE`
      снимать сразу после каждой команды.
- [x] **Step 2:** стенд `gca_dev`: завести ряд «Росстат, ИПЦ, декабрь к декабрю»
...
- [x] **Step 3:** проверить браузером (playwright-core, `channel: "chrome"`):
...
- [x] **Step 4:** пересобрать макет командой из §7 спеки, если стенд изменился.
- [x] **Step 5:** соответствие «требование спеки → тест» построить **списком по
...
- [x] **Step 6:** devlog `docs/devlog/2026-08-18-inflation-adjustment.md`: что
