...
### Разрешение дат — отдельная функция, отдельный запрос

`resolve_inflation(db, contract_ids, *, series_id, target_month)` читает
`estimates.data_prepared_on_date` вместе с `contracts.signed_date` **одним своим
запросом** и возвращает план приведения либо бросает `DomainError` с кодом.
Она НЕ встроена в `_load_estimates` по двум причинам:
...
* `load_rollups` держит инвариант «ровно четыре запроса на любую выборку», и его
  ...
  с жёстким `assert c3.total <= 4`. Пятый запрос внутри `load_rollups` уронил бы
  его. Запрос `resolve_inflation` живёт в `build_comparison`, выполняется
  **только когда приведение включено** и от размера выборки не зависит — тест
  `test_comparison_buckets.py::test_build_comparison_query_count_does_not_grow_with_selection`
  сравнивает числа между собой, а не с константой, и остаётся зелёным.
...
1. **Коэффициент колонки, когда сметы договора приведены РАЗНЫМИ множителями.**
   ...
   договора получили один и тот же коэффициент**; иначе `null`, и чип
   показывает «разные» с подсказкой «коэффициент считается по каждой смете;
   см. корзины ДГП и ДС». Врать одним числом там, где их два, нельзя, а вторая
   ось чипов удвоила бы шапку. На стенде случай не воспроизводится (ДС нет) —
   проверяется фикстурой.
...
4. **Пятый запрос при включённом приведении** — `resolve_inflation` (см.
   «Архитектура»). Инвариант «не растёт с выборкой» сохраняется.
...
- [ ] **Step 2:** реализация в указанной форме.
- [ ] **Step 3:** тест атомарности: `PATCH` с двумя годами, где ПЕРВЫЙ валиден
      (2025, `1.0830`), а ВТОРОЙ невалиден (2026, `coefficient = "-1"`), плюс
      новое `name`. Ожидание: `422`, в базе `name` прежнее, года 2025 нет,
      года 2026 нет.
...
одноимённые схлопываются молча).
...
- [ ] **Step 2:** умножение в `load_rollups`, план — из задачи 7.
- [ ] **Step 3:** `_mode_caption` получает `InflationPlan | None` и дописывает
...
- [ ] **Step 4:** `columns[].inflation_coefficient` по правилу решения плана №1.
- [ ] **Step 5 — НЕ ДЕЛЕГИРУЕТСЯ, снятие защиты (DoD 31):** зашить название ряда
...
**Утверждения с числами** (фикстура: три договора, статья «1», нетто 1 000 000 /
1 000 000 / 1 000 000, площадь 100 000 м², ряд `{2024: 1.075, 2025: 1.083,
2026: 1.060}`, цель `2026-08`, даты смет 2024-05-28 / 2025-02-20 / 2026-07-06):
...
| номинальный ответ | ключа `inflation` нет, `inflation_coefficient` нет — golden задачи 1 зелёный | 1 |
...
- [ ] **Step 3:** проп `missingYears?: number[]` — вход из баннера открывает окно
      с уже добавленными пустыми строками этих годов.
- [ ] **Step 4:** архивный ряд: кнопка правки **выключена**, в шапке окна
      пояснение «сначала вернуть в активные».
- [ ] **Step 5:** компонентный тест DoD 25: ввести `0.083` в поле коэффициента и
      увидеть «Снижение 91,7 %» **до сохранения**.
- [ ] **Step 6 — НЕ ДЕЛЕГИРУЕТСЯ, снятие защиты (DoD 25):** удалить вызов
...
- [ ] **Step 1:** третья вкладка рядом со «Ставки» и «Классы объектов»: таблица
      рядов (название, примечание, охват годов, состояние, кнопка «Изменить»),
      кнопка «Создать ряд». Архивные показываются с пометкой и выключенной
      кнопкой.
- [ ] **Step 2:** MSW-хендлеры рядов в `test/handlers.ts` + фикстура двух рядов
      и одного архивного в `test/fixtures.ts`.
- [ ] **Step 3:** тесты: список показывает охват «2024–2026»; «Изменить»
      открывает **тот же** `InflationSeriesDialog`; у архивного кнопка выключена.
...
- [ ] **Step 4:** чип коэффициента в шапке колонки — **уровень** `+17,4 %`,
      ...
      null` — «разные» с подсказкой (решение плана №1).
- [ ] **Step 5:** `InflationRefusalBanner` — при `422` с кодом: показать
      причину и недостающие годы, дозапросить номинальный вариант и показать
      его, **параметры URL сохранить**, переключатель визуально выключить,
      кнопка «Заполнить недостающие годы» открывает тот же диалог с этими годами.
