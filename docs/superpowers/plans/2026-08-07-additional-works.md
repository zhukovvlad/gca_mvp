...
- Modify: `backend/models.py` (новый класс; константа пробельных символов)
...
- [ ] **Step 5: Зелёный прогон схемных тестов + `alembic check`**
...
- [ ] **Step 6: Круговой рейс — прогнать, а не объявить**
...
```python
SVEDENIYA_KEY_RE = re.compile(r"(?i)сведени.*дополнительн")
LINE_RE = re.compile(
...
def svedeniya_text(additional_info: Mapping[str, Any]) -> str | None: ...
...
- `svedeniya_text` находит ключ по регэкспу; пустое значение и отсутствие ключа дают `None` (одинаково — это одно состояние, спека §1.1).
...
Посчитан по этому плану, а не оценкой: Task 1 — 14 (девять функций, три параметризованы), Task 2 — 4 новых плюс два переименованных, Task 3 — 17, Task 4 — 12, Task 5 — 10, Task 6 — 8. Итого **65**, база 1029 → ожидание **1094**. Авторитетен замер `pytest --collect-only -q`; расхождение с 65 означает, что план и реализация разошлись, и разбираться надо **до** PR, а не подгонять число.
