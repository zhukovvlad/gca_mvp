...
Роли зафиксированы пользователем: **Opus — оркестратор**, **Sonnet/Haiku — исполнители** (по сложности задачи), **Fable — финальное ревью** перед мержем. Коммиты делает оркестратор.

| Работа | Кому | Почему |
|---|---|---|
| Task 1 целиком | Sonnet | новые функции, поиск строки шапки, тексты отказов, семь тестов — есть суждение |
| Task 2 Step 0 | Sonnet | переезд исключения задевает публичный контракт `parser.EstimateParseError` |
| Task 2 Steps 1–5, 7–8 | Sonnet | dataclass, распознавание, разводка в `get_proposals`, версия |
| Task 2 Step 6 (14 вызовов `.positions`) | **Haiku** | чистая механика без решений |
| Task 1 Step 5 (шапка и `A11/B11` в фикстуры) | **Haiku** | то же, если оркестратор захочет вынести отдельным вызовом |
| Task 3 Steps 1–2, 4–6 | Sonnet | прогоны, замер по офертам, devlog, PR |
| Task 3 Step 3 | **оркестратор лично** | негативная проверка, там легко получить ложно-зелёный |
| Финальное ревью ветки | Fable | диффы, devlog, вывод негативных проверок |

Оркестратор **не делегирует**: негативную проверку Task 3 Step 3, решение при любом расхождении с планом и любые правки спеки.
