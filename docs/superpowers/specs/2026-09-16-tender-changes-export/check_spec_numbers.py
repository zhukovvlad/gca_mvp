...
def claims(data: dict) -> list[tuple[str, str, str]]:
    ...
    #: украшение: `den` стоит под фильтром пригодности, и разницу между ним и
    #: физическим объёмом показывает соседний счётчик `mix_priced_set`.
    mix_volume_exact = [spaced(t["mix_volume_exact"]) for t in (second, main, third)]
    #: Внутри `mix_volume_move` — пары, где `den` двинула САМА ПРИГОДНОСТЬ
    #: строки, а не объём. Заведено потому, что прежняя редакция §2.7 говорила
    #: про этот механизм «их ноль», а ноль был НЕДОСТИЖИМОСТЬЮ: строка, вошедшая
    #: в расценённое множество, всегда двигает `den`, и в счётчик «иная причина»
    #: (он требует РАВНОГО `den`) такая пара попасть не могла по устройству.
    mix_priced_set = [spaced(t["mix_priced_set_move"]) for t in (second, main, third)]
    ...
    out = [
        ...
        ("из них — двинулось расценённое множество",
         ", ".join(mix_priced_set[:2]) + " и " + mix_priced_set[2],
         "расценённое множество двинулось в {}"),
