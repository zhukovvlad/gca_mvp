...
def trace_measurements(conn, tender: int, participant: str) -> dict:
    ...
    #: сырые суммы разошлись, а настоящего движения состава не было.
    #: `mix_trigger_total` — все такие пары; внутри них разложение ПО ПРИЧИНЕ,
    #: и причина проверяется, а не предполагается (внешнее ревью, круг 3):
    #:   • `mix_volume_move` — обе стороны сравнимы, состав на единицу РАВЕН
    #:     И ИСПОЛЬЗУЕМЫЙ ОБЪЁМ (`den`) ИЗМЕНИЛСЯ. Именно этот механизм
    #:     цитирует §2.7. Прежняя редакция счётчика не сравнивала ни `qty`,
    #:     ни `den` — она считала «обе стороны сравнимы» и называла это
    #:     движением объёма, хотя абсолют мог разойтись, например, из-за
    #:     строки, непригодной по цене: в абсолютную тройку такая строка
    #:     входит, в удельную — нет.
    #:   • `mix_volume_other` — остаток того же подмножества: сравнить было
    #:     можно, состав на единицу равен, а объём НЕ ИЗМЕНИЛСЯ, то есть
    #:     сработала иная причина. Печатается всегда: пока он ноль, спека
    #:     вправе называть механизм объёмом; как только перестанет — число
    #:     в тексте разойдётся с базой и страж покраснеет. ВАЖНО, чего этот
    #:     ноль НЕ значит: строка, вошедшая в расценённое множество, ВСЕГДА
    #:     двигает `den` (её `qty` стоит под тем же `FILTER`), поэтому такая
    ...
    #:     это копейки исходной сметы, и текст вправе так и сказать.
    #: Направление объёма НЕ ОДНО (на стенде есть и рост, и убыль), поэтому
    #: разбиение `den_up/den_down` печатается И ВХОДИТ В СТРАЖА: утверждение
    #: «есть и рост, и убыль» — тоже замер. Спека говорит «изменение объёма»,
    #: а не «рост», ровно потому, что направление измерено.
    #: Считаются ПОПАРНО, а не арифметикой `abs_move - unit_move`: та разница
    #: честна как отдельное число, но занижает счёт на редкую границу, где
    #: сырые суммы СОВПАЛИ, а состав на единицу всё равно разошёлся (объём
    #: изменился при неизменных абсолютах, суженный ТОЛЬКО набором изменённых
    #: полей) — ровно один такой случай есть на трассе «3 · АНТТЕК» (внешнее
    ...
    for series in by_work.values():
        for prev, cur in zip(series, series[1:]):
            ...
            if abs_differs and not unit_differs:
                mix_trigger_total += 1
                if comparable:
                    # Причина расхождения — движение ИСПОЛЬЗУЕМОГО объёма:
                    # знаменателя, на который делится удельный состав. Равные
                    # доли при другом `den` дают другие абсолюты — это и есть
                    # механизм §2.7. Равный `den` означает иную причину.
                    if cur["den"] != prev["den"]:
                        mix_volume_move += 1
                        if cur["den"] > prev["den"]:
                            mix_den_up += 1
                        ...
                        if all(side[f"unit_{n}"] == side[n]
                               for side in (prev, cur) for n in TRIPLE):
                            mix_volume_exact += 1
                        else:
                            mix_exact_gap = max(
                                mix_exact_gap,
                                max(abs(side[f"unit_{n}"] - side[n])
                                    for side in (prev, cur) for n in TRIPLE))
                    else:
                        mix_volume_other += 1
                else:
                    mix_not_comparable += 1
...
def main() -> None:
    ...
    for t in data["traces"]:
        ...
        print(f"  признак по абсолюту сработал бы ошибочно: {t['mix_trigger_total']} пар "
              f"(сравнивать на единицу нечего: {t['mix_not_comparable']}; "
              f"изменение используемого объёма при неизменном составе: "
