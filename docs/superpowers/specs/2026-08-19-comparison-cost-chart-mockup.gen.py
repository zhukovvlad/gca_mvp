...
// Ось считается по состоянию, КОТОРОЕ НАСТУПИТ: иначе «Привести» переписывало бы
// подписи засечек вместо того, чтобы растить столбцы.
const axisState = () => sid ? sid + '@' + (month || D.defMonth) : '-';
...
function render() {
  ...
  const axis = D.top[ind + '|' + vat + '|' + axisState() + '|' + subset()];
  ...
  <section>
    ...
    <div class="card card-pad">
      <ul class="notes">
        ...
        <li><b>Ось не пересчитывается при включении приведения.</b> Верх оси
        считается сразу по номиналу и по выбранному ряду. Иначе при нажатии
        «Привести» столбцы остались бы на месте, а поехали бы подписи засечек —
        главный эффект фичи стал бы невидимым. Плата: масштаб меняется в момент
        выбора ряда, когда числа ещё прежние.</li>
