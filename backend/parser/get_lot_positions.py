...
class LotRows:
    ...
    Ф4 (спека §2.1): строка живёт только здесь. В `positions` её нет вовсе —
    переходное решение Ф2, державшее строку в обоих местах, снято.
    """
...
def get_lot_positions(ws: Worksheet, contractor: dict[str, Any], lot_start_row: int, lot_end_row: int) -> LotRows:
    ...
    она копируется в `LotRows.additional_works` и в `positions` НЕ ПОПАДАЕТ
    (Ф4, спека §2.1: дубль не удаляется, а не создаётся). Любое другое название
    такой строки — структурный отказ, как и вторая подобная строка в пределах
    лота.
    ...
    for current_row_num in range(lot_start_row, lot_end_row + 1):
        ...
        if cell_text_is_blank(item[JSON_KEY_NUMBER]) and cell_text_is_blank(item[JSON_KEY_CHAPTER_NUMBER]):
            ...
            # Ф4 (спека §2.1): дубль не снимается удалением — он не создаётся
            # вовсе. Строка уже скопирована в additional_works выше, и
            # `continue` не даёт ей попасть в positions. Непрерывность ключей
            # 1..N сохраняется по построению: item_index инкрементируется
            # только вместе с записью в словарь (строка ниже), поэтому
            # пропуск записи здесь не оставляет дыры в нумерации.
            continue
