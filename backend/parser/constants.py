...
* ключи результирующей JSON-структуры (точный результат уходит в
  `import_jobs.parsed_data`, под сметой в виде проекции — в
  `estimate_raw_data.raw_data`; разбирается импортом фазы 4 — переименовывать
  ключи нельзя);
