...
#
# 2.0.0 (Ф4): агрегатная строка допработ исчезла из `positions` — она и раньше
# дублировалась в `additional_works`, но теперь `positions` её не несёт вовсе.
# Это ломающее изменение контракта (не появление ключа — оно уже случилось в
# 1.1.0), поэтому мажор, а не минор. Потребителя у поля по-прежнему нет ни в
# одной строке кода, но это единственный способ отличить старый разбор от
