...
class RoundOverridePut(BaseModel):
    ...
    # решение пользователя 02.09.2026: экран уже схлопывает пустое поле в null
    # перед отправкой, нормализация на сервере делает это частью контракта, а
    # не любезностью клиента, которую легко забыть у другого потребителя.
