...
def download_import_job_file(
    ...
    настроенного срока). Запись задания остаётся, пока жив договор, это аудит
    (v6.7); поэтому «нет файла» и «нет задания» — разные ответы.
    """
    ...
    try:
        ...
        raise HTTPException(
            ...
            "Сама запись задания остаётся, пока жив договор.",
        ) from exc
