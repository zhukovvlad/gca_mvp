...
Права: чтение — любому аутентифицированному (`passport_top_n` читает паспорт ФАЗЫ 6
из легаси-аналитики, а он доступен и `member`), изменение — `admin` по §3
(«admin — управление пользователями, классами, нормативами…»: настройка печатной
формы того же рода). Само чтение настроек не admin-операция независимо от того, кто
её потребитель.
...
class SettingsUpdate(BaseModel):
    ...
    **Границы диапазона здесь намеренно НЕ объявлены** (`Field(ge=…, le=…)` убран).
    ...
    граница подобрана под раскладку экрана паспорта фазы 6, а не взята произвольно.
    Ограничение осталось бы продублированным в трёх местах, а работал бы из них
    худший.
