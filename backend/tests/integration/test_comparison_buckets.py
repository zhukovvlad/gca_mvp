...
def test_apply_deviation_is_a_noop_only_when_the_median_is_zero():
    """`_apply_deviation` пропускает счёт РОВНО когда медиана нулевая, не шире
    и не уже (защита старше фичи предиката цены; редакция и переименование
    теста — задача 8 плана `2026-09-09-price-predicate-design.md`, находки
    ревью).

    **Смысл теста сменился, не только числа — и имя обязано это отражать**
    (ревью, правка 5: старое имя обещало строку и неупавший СВОД, а здесь
    после переписывания нет ни строки, ни свода, ни API — есть словарь из
    двух ячеек и вызов функции). До фичи предиката цены этот тест строил
    нулевую медиану ЧЕРЕЗ API (`build_comparison`): вход допускал
    отрицательную ячейку как сравнимую, и пара `-1200.00`/`-1200.00` вместе с
    `1200.00`/`1200.00` усредняла медиану РОВНО в ноль. Отбор сравнимых ячеек
    теперь идёт через `is_price` (конечно и строго больше нуля, задача 8), и
    эта фикстура так же перестаёт быть сравнимой сама — комплект уже
    используется по её новой роли в `test_comparison_medians.py::
    test_negative_net_per_sqm_is_excluded_and_the_median_disappears`, где то
    же самое исключение проверяется на входе API (а НЕ как источник
    «отрицательное представимо» для того теста — это ссылалось на текст,
    который сама же данная правка стёрла; независимое основание — отсутствие
    `CHECK` на знак у `position_items.total_cost_total`). Здесь у теста
    другая задача: не «дать API нулевую медиану» (это теперь никаким входом
    не получится), а «защитить функцию `_apply_deviation` от нулевой медианы
    вообще», и это ДРУГОЕ утверждение.

    **Почему через API это состояние стало НЕДОСТИЖИМО — проверено замером,
    а не выводом; независимо перепроверено вторым ревьюером.** Между
    `_compute_median` и `_apply_deviation` нет ни одного `.quantize()`/
    `round()` (`grep` по `crud/comparison.py` — ноль совпадений), значит
    малая положительная величина не может округлиться в ноль на пути к
    проверке. JSON-кодировщик ответа (`responses._decimal_encoder`,
    `format(x, "f")`) тоже не округляет — теряется только экспоненциальная
    запись, не разряды. `_DIV_CONTEXT` — `Context(prec=100, Emin=-999999,
    ...)`: замерено, что среднее ДВУХ различных строго положительных
    `Decimal`, вплоть до экстремальных (`(Decimal("1E-90") +
    Decimal("3E-90")) / 2 == Decimal("2E-90")`, не `0`), не даёт точный `0` в
    этом контексте. `Underflow` этот контекст НЕ трапит (в списке трапов его
    нет) — но входные `net_per_sqm` берутся из `numeric` PostgreSQL, а его
    диапазон представимых величин обрывается на экспоненте порядка ±131072,
    тогда как `Etiny` контекста при `Emin=-999999, prec=100` — около
    `-999999-100=-1000099`: запас в СЕМЬ порядков экспоненты, поэтому
    `Underflow` до нуля недостижим на входах, которые вообще может произвести
    база. По той же причине недостижимо и переполнение вместо деления на
    ноль — `Overflow` в списке трапов есть, но диапазон `numeric` не
    достигает границы `Emax` контекста на суммах и площадях этого масштаба.
    Медиана, построенная целиком из `is_price`-пригодных значений — при
    нечётном числе сравнимых это САМО одно из них, при чётном — среднее ДВУХ
    положительных, — математически не может быть нулём. Значит через
    `build_comparison` эту защиту предъявить больше нечем.

    **Недостижимо СЕГОДНЯ — не то же самое, что недостижимо НИКОГДА**
    (`docs/insights/verifying-guards.md`, слой 10). `_apply_deviation` — код
    общего назначения, а не только для текущего вызывающего: следующая
    правка (ослабление `is_price`, новый вызывающий код, прямой вызов извне)
    способна снова дать ему нулевую медиану, и без проверки строка получила
    бы `DivisionByZero` и `500` вместо прочерка. Поэтому защита остаётся, а
    тест переведён на прямой вызов `_apply_deviation` со СКОНСТРУИРОВАННОЙ
    нулевой медианой — она утверждает о ФУНКЦИИ, а не о сегодняшнем пути к
    ней извне.

    **Утверждение записано ЭКВИВАЛЕНТНОСТЬЮ, а не одной импликацией** (ревью,
    правка 4; `docs/insights/state-the-rule-as-an-equivalence.md`).
    Половинчатая версия — только «нулевая медиана не падает» — не отличает
    «защита сработала корректно» от «расчёта не было вовсе»: мутация «сделать
    функцию безусловным no-op (всегда возвращать `cells` как есть, выбросив
    расчёт целиком)» оставляла бы такую версию ЗЕЛЁНОЙ, потому что
    единственное её утверждение уже выполнено на ячейках, изначально созданных
    с пустым `deviation_pct`. Поэтому здесь ДВЕ ветки на ОДНИХ И ТЕХ ЖЕ
    ячейках: нулевая медиана -> отклонение НЕ считается (`None`); ненулевая
    медиана -> отклонение СЧИТАЕТСЯ и равно ожидаемому числу — вторая половина
    ловит именно ту мутацию, которую первая пропускала.
    """
    def cell(net_per_sqm: Decimal) -> cmp.BucketCell:
        return cmp.BucketCell(
            net=net_per_sqm * 100, shown=net_per_sqm * 100, net_per_sqm=net_per_sqm,
            shown_per_sqm=net_per_sqm, state=cmp.VALUE, deviation_pct=None,
            incomplete_reasons=frozenset(),
        )

    cells = {1: cell(Decimal("10")), 2: cell(Decimal("20"))}

    zero_median = cmp._MedianResult(value=Decimal("0"), comparable_count=2, contract_ids=[1, 2])
    zero_result = cmp._apply_deviation(cells, zero_median)
    for contract_id in (1, 2):
        assert zero_result[contract_id].deviation_pct is None, (
            "нулевая медиана: отклонение не определено — ячейка без подсветки, "
            "а не деление на ноль"
        )

    real_median = cmp._MedianResult(value=Decimal("10"), comparable_count=2, contract_ids=[1, 2])
    real_result = cmp._apply_deviation(cells, real_median)
    assert real_result[1].deviation_pct == Decimal("0"), (
        "ненулевая медиана: ячейка 10 при медиане 10 -> отклонение 0% — СЧИТАЕТСЯ "
        "(не None), это и есть вторая половина эквивалентности"
    )
    assert real_result[2].deviation_pct == Decimal("100"), (
        "ненулевая медиана: ячейка 20 при медиане 10 -> отклонение 100% — расчёт "
        "реально произошёл, а не был пропущен безусловно"
    )
