...
def _wait_until_backend_blocks(
    ...
    решение оркестратора для задачи 4.
    """
...
class TestChapterCycleGuard:
    def test_cycle_in_chapter_item_id_raises_loudly(
        ...
        именно так и потерялось 70 минут при ревью. Поэтому вызов идёт в
...
class TestForShareLockContract:
    def test_existing_for_update_delays_routing_until_released(
        ...
        try:
            ...
            # совершенно другом тесте (решение оркестратора).
            try:
    ...
    def test_for_share_is_compatible_with_itself(
        ...
        try:
            ...
            # сталкиваются с TRUNCATE в teardown следующего теста (решение
            # оркестратора).
            release.set()
            ta.join(timeout=10)
            tb.join(timeout=10)
    ...
    def test_route_position_itself_is_compatible_with_itself(
        ...
        try:
            ...
            # teardown становится его жертвой (решение оркестратора).
            release_a.set()
            ta.join(timeout=20)
            if tb_started:
                tb.join(timeout=20)
...
class TestBucketIdempotentUnderConcurrency:
    def test_two_concurrent_imports_of_the_same_pair_yield_one_bucket_and_one_context(
        ...
        try:
            ...
            # следующего теста (решение оркестратора).
            a_release.set()
            ta.join(timeout=20)
            if tb_started:
                tb.join(timeout=20)
