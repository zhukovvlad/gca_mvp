...
только тот, до которого её сузили (спека диаграммы стоимости §2.7, DoD 8).
...
def test_facet_defaults_to_contract_ids_when_omitted(db_session, factories):
    """`build_comparison` без `facet_ids` считает фасет по самой выборке.

    Умолчание `None` означает «сужения не было», а НЕ «фасета нет»: фасет обязан
    присутствовать всегда (DoD 7), и когда никто не сужал, множество до сужения и
    есть сама выборка. Это условие совместимости для всех прямых вызовов
    `build_comparison`, которых в проекте больше шести десятков.
    """
    contract_ids, shared_id, other_id = fx.contracts_sharing_a_rate_class(db_session, factories)
...
def test_facet_ordered_by_title_stable_and_independent_of_columns(db_session, factories):
    ...
    совпадении, ничего не проверив. С заголовками «А», «Б», «В», созданными в том
    же порядке, этот тест зелен при ЛЮБОЙ реализации сортировки.
    """
