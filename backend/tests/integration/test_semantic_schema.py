...
def _live_check_expr(session, table: str, constraint_name: str) -> str:
    ...
    Константу можно оставить множеством, подменив предикат
...
# ---------------------------------------------------------------------------
#  4b. Пробой на каждый CHECK, у которого пробоя не было
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
#  6b. CK_EVENT_SUBJECT_BY_TYPE — вычислен СТАНДАЛОНОМ, вне таблицы
# ---------------------------------------------------------------------------

class TestEventSubjectByTypeExpression:
    """Доказывает, что ограничение,
    ...
    Читать текст из `models.CK_EVENT_SUBJECT_BY_TYPE`/константы миграции
    ...
    def test_family_type_set_matches_independent_literal_from_db_text(self, db_session):
        ...
        БД: PostgreSQL нормализует `IN (...)` в
...
class TestDowngradeRefusalLogic:
    """`_downgrade_refusal` проверена ЧИСТОЙ ЛОГИКОЙ, на
...
class TestDowngradeBlockers:
    """Живые тесты сверяют ЦЕЛИКОМ словарь `blockers`, и
    ...
    def test_downgrade_then_upgrade_round_trip_on_clean_scratch_db(self):
        ...
        try:
            ...
            try:
                ...
                # Сам downgrade обязан быть под замком — убрать
...
# ---------------------------------------------------------------------------
#  Частичные индексы context_members — предикат закреплён буквально
# ---------------------------------------------------------------------------
...
class TestParityWithMigration:
    """Parity покрывает КАЖДЫЙ CHECK шести таблиц, а не только одиннадцать
    ...
    список событий) тоже CHECK, и без этого расширения ничто не ловило их
    расхождение между `models.py` и миграцией."""

    #: `ids=` — имя константы, а не длинная строка значения, чтобы
    #: упавший случай назывался по имени.
    _CASES = [
