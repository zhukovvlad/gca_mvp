...
class UnitResolver:
    ...
    и **алиасы** из `unit_aliases`.

    **Правило разрешения конфликтов одно на всех: первая заявка на форму
    побеждает, конфликтующая — отклоняется и логируется.** Регистрация идёт
    детерминированным порядком — сначала канонические формы (по `id` единицы),
    затем алиасы (по `id` алиаса), — поэтому каноническая форма всегда сильнее
    алиаса чужой единицы.

    Почему не «алиас авторитетнее, он заведён человеком»: собственное название
    единицы — это её идентичность. Алиас «Кв. метр» → `M3` сделал бы `M2`
    недостижимой по её же имени, и позиции в квадратных метрах молча уехали бы в
    кубические. Человек, заводящий алиас, не может знать, что он отнимает имя у
    другой строки справочника, — а последствие невидимо: `unit_id` входит в
    идентичность каталожной работы (§4). Само по себе такое столкновение —
    дефект справочника, и правильный ответ на дефект не «выбрать сторону молча»,
    а отказать и сказать вслух. Отклонять такой алиас на входе — задача CRUD
    единиц в фазе 5.
    ...
    **Решение фазы 4 по неизвестной единице** (`docs/phase4-import.md` §2.3):
    ...
    def __init__(self, db: Session) -> None:
        ...
        for raw_text, unit_id, code in aliases:
            self._register(normalize_unit_key(raw_text), unit_id, code, source="алиас")

    def _register(self, key: str, unit_id: int, code: str, *, source: str) -> None:
        """Заносит форму записи единицы, не затирая чужую и не молча.

        Столкновение форм двух РАЗНЫХ единиц — всегда дефект справочника:
        одинаковое название у двух строк, алиас, повторяющий чужое каноническое
        имя, или два алиаса, различающиеся только регистром (`unit_aliases.raw_text`
        уникален по сырому тексту, а не по нормализованному ключу). Тихо выбрать
        одну сторону значило бы привязывать позиции к произвольной единице,
        поэтому побеждает первая заявка, а расхождение попадает в лог.

        Повторная заявка ТОЙ ЖЕ единицы конфликтом не считается: алиас «м2» при
        символе «м²» — это одна и та же форма, а не спор.
        """
        if not key:
            return
        claimed = self._by_key.get(key)
        if claimed is not None:
            if claimed[0] != unit_id:
                log.warning(
                    "Форма единицы «%s» (%s единицы %s) уже занята единицей %s и "
                    "применена НЕ будет: %s останется недостижимой по этой форме. "
                    "Это дефект справочника единиц — уберите дубль.",
                    key,
                    source,
                    code,
                    claimed[1],
