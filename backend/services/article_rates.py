...
def fold_carrier_rows(rows: Sequence[CarrierRow], effective_rate: Decimal | None) -> ArticleFold:
    """Свёртка строк-носителей статьи в одну сумму, объём и признаки годности.

    Строка со статусом `AmountStatus.UNKNOWN_BASE` (база НДС неизвестна) и
    непустой суммой — ГОДНОЕ слагаемое: `restate_gross` в этом случае отдаёт
    исходный валовый `amount` нетронутым, и он идёт в накопление как есть, без
    приведения к ставке показа. Решение НАМЕРЕННОЕ и согласовано с
    `_direct_totals` (`crud/project_passport.py`): та функция берёт
    `restated.amount` при ЛЮБОМ статусе для каждой денежной цифры паспорта, в
    том числе для «Итого, ₽» рядом со ставкой — ставка обязана вести себя так
    же, а более строгое правило здесь напечатало бы «сумма не прочитана»
    рядом со строкой, где тот же самый рубль уже показан итогом.
    """
    if not rows:
        return ArticleFold(
            rows=0,
...
def resolve_rate(
    ...
    Смешение единиц детей (проверка 10, `convergence_note`) ловится ПРЕДИКАТОМ
    ...
    Нулевая сумма своей свёртки (проверка 9, `AMOUNT_ZERO`, §2.10) стоит ПОСЛЕ
    восьми проверок собственных входов свёртки (числитель, единица, знаменатель)
    и ДО сходимости детей: числитель, равный нулю, — такой же собственный вход,
    как и остальные восемь, а не факт о дереве. Рядом с `amount_missing` (проверка
    3, «сумма не прочитана») она встать не может: обе проверки о числителе, но
    там, где сумма НЕ ПРОЧИТАНА, и там, где сумма ПРОЧИТАНА и равна нулю, —
    разные факты, и более ранняя позиция обесценила бы более содержательные
    подписи `unit_missing`/`unit_not_scalable`/`volume_missing` на свёртках,
    которые сегодня до них доходят (§2.10, отвергнутая альтернатива «рядом с
    проверкой 3»). После сходимости детей (проверка 10) она тоже встать не
    может: свёртка с нулевой суммой и несходящимися детьми обязана называть
    первым делом отсутствие денег статьи, а не разногласие детей, которое эти
    деньги не считает вовсе.
    """

    def _dead_end(state: RateState) -> ArticleRate:
        """Тупиковая ветка (проверки 1–9, включая новую `AMOUNT_ZERO`): ставки
        нет, `note` нет — он есть только у `VOLUME_INCONSISTENT` (проверка 10),
        а `volume`/`unit_rate` нет ни у одной ветки, кроме `RATE` (проверка 11).
        `unit` — всегда `fold.unit_symbol`: `fold_carrier_rows` уже гарантирует,
        что он `None` ровно там, где единица неизвестна или конфликтна, так что
        отдельного условия под каждое из девяти состояний не требуется."""
    ...
    # Девятая проверка, последняя среди девяти входных (§1.9, §2.9, §2.10):
    # сумма СВОЕЙ свёртки конечна (гарантия `amount_ok` выше — `fold.amount`
    # здесь не `None` и не NaN/Infinity, сравнение с нулём не бросает
    # `InvalidOperation`) и равна нулю. `Decimal("-0.00") == 0` истинно тем же
    # оператором, которым Decimal вообще сравнивает величины, — отрицательный
    # ноль здесь СЧИТАЕТСЯ нулём осознанно: он тот же факт «денег нет», а не
    # отдельный знак, который стоило бы отличать.
    if fold.amount == 0:
        return _dead_end(RateState.AMOUNT_ZERO)
