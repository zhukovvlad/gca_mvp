...
#: Проверяется НА ВХОДЕ обеих публичных функций,
...
def lock_buckets(db: Session, bucket_ids: list[int], *, exclusive: bool) -> None:
    ...
    Raises:
        RoutingError: среди `bucket_ids` есть значения, которых нет в
            `context_buckets` — называет ИМЕННО
...
def _get_or_create_bucket_with_default_context(
    ...
    *,
    ...
    контекст по умолчанию — в ОДНОЙ транзакции: новая
...
def route_position(
    ...
    """Маршрутизирует одну позицию (спека §2.4). `origin` —
    значение, записываемое в `context_created.origin`, если
    ...
    Raises:
        RoutingError: `origin` не входит в допустимое множество (проверяется
            ПЕРВЫМ, всегда), позиция не найдена, это
...
def route_positions(
    ...
    Порядок захвата локов: СНАЧАЛА разрешаются (и при
    ...
    Raises:
        RoutingError: `origin` не входит в допустимое множество — проверяется
            ПЕРВЫМ, всегда, даже если `estimate_ids` пуст; либо любой из отказов `route_position`,
    ...
    # Партия обрабатывается по ВОЗРАСТАНИЮ `catalog_position_id` (затем
    # `id` позиции — тай-брейк на одинаковой каталожной строке, детерминизм
    # без побочного эффекта на исход), а не в порядке прихода `positions` —
    # у ЭТОГО запроса нет `ORDER BY`, поэтому без сортировки здесь порядок
    # обработки партии совпадает с порядком физического хранения строк
    # `position_items`, который ничем не связан с id каталожных строк, на
    # которые они ссылаются. Обработка новой (создающей корзину) позиции
    # берёт `FOR SHARE` на ЕЁ каталожную строку внутри
    # `_create_default_context` (`_lock_catalog_position_share`) — ОДНОЙ
    # строкой за раз, а не пакетом, поэтому порядок обработки партии — это и
    # есть порядок захвата этих локов. Слияние в Review (`services/review.py
    # ::_lock_rows`) берёт `FOR UPDATE` на каталожные строки ОДНИМ запросом
    # `ORDER BY id`, то есть тоже по возрастанию: если партия обрабатывала бы
    # позиции в обратном порядке, набор локов партии шёл бы ВСТРЕЧНЫМ
    # направлением — импорт держит бо́льший id и ждёт меньший, слияние держит
    # меньший и ждёт бо́льший, и PostgreSQL обязан аварийно прервать одну из
    # сторон дедлоком.
    eligible.sort(key=lambda position: (position.catalog_position_id, position.id))
