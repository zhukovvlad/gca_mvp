...
def _replace_existing(
    ...
    # `lock_estimate` в `services/category_override.py` на ТОЙ ЖЕ строке
...
def _import_positions(
    ...
    *,
    ...
    for position_key, raw_position in positions.items():
        ...
        # `continue` каскада матчинга (см. разбор в отчёте задачи 7).
        domain_value_problems.extend(
            _price_domain_warnings(item.unit_cost_total, item.total_cost_total, where)
        )
