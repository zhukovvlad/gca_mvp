...
В MVP настройка одна — `passport_top_n`, число «ключевых расценок» ПАСПОРТА ФАЗЫ 6
(`GET /api/v1/analytics/passport/{contract_id}`; паспорт проекта Ф6 фазы 7 от неё не
зависит — спека §2.6). §7.4 требует хранить её **в БД**: «N хранится в БД (таблица
...
def get_passport_top_n(db: Session) -> int:
    """N для «ключевых расценок» ПАСПОРТА ФАЗЫ 6 (§7.4).

    Точка входа для `GET /api/v1/analytics/passport/{contract_id}`. Паспорт проекта
    Ф6 фазы 7 (`analytics/project-passport`) этой настройки не читает вовсе.
    """
...
def update_settings(db: Session, *, passport_top_n: int) -> dict:
    ...
    if not PASSPORT_TOP_N_MIN <= passport_top_n <= PASSPORT_TOP_N_MAX:
        raise DomainError(
            ...
            f"{PASSPORT_TOP_N_MAX}. Верхняя граница — не прихоть: она подобрана под "
            "раскладку экрана паспорта фазы 6, а не взята произвольно.",
        )
