...
def _is_incomplete(rows: Sequence[_TotalsRow]) -> bool:
    ...
    **Третье условие спеки (`rows_not_finite > 0`) ВТОРЫМ ПОГЛОЩЕНО, и отдельной
    ...
    `rows_with_amount < row_count`, и третья ветка недостижима. `AGENTS.md` §11
    запрещает оборонительный код против недостижимого входа, поэтому её здесь
    нет; сама импликация закреплена тестом
