...
def _is_incomplete(rows: Sequence[_TotalsRow]) -> bool:
    """Неполнота §2.6. Условия ДВА, хотя спека называет три, — и это не пропуск.

    **Первое обязательно и из второго НЕ выводится.** Позиционная ветвь VIEW
    строится `WHERE pi.is_chapter = false`, поэтому смета без единой нераздельной
    позиции (пустая либо состоящая только из разделов) не даёт ни одной строки.
    `SUM` по пустому множеству равен `NULL`, а не нулю: сравнение
    `rows_with_amount < row_count` даёт `NULL`, то есть ЛОЖЬ, и договор без
    единой суммы прошёл бы как полный.

    **Третье условие спеки (`rows_not_finite > 0`) ВТОРЫМ ПОГЛОЩЕНО, и отдельной
    ветки не получает** (находка ревью Codex; проверено снятием — с удалённой
    третьей веткой весь набор оставался зелёным, потому что уронить её нечем).
    Причина в самом VIEW: `rows_with_amount` считается фильтром, который
    исключает `NaN`/`±Infinity` наравне с `NULL`, а `row_count` — это `COUNT(*)`
    без фильтра. Значит `rows_not_finite > 0` ВСЕГДА влечёт
    `rows_with_amount < row_count`, и третья ветка недостижима. Ветка, которую
    нечем уронить, — это ветка без защиты (`docs/insights/verifying-guards.md`
    §7: «если снятие не валит ничего — защиты нет»), поэтому её здесь нет; сама
    импликация закреплена тестом
    `test_view_keeps_non_finite_rows_out_of_rows_with_amount` — если VIEW
    когда-нибудь начнёт считать нефинитные строки как ценённые, тест покраснеет
    и потребует вернуть условие сюда.
    """
    row_count = sum(row.row_count for row in rows)
    if row_count == 0:
        return True
    return sum(row.rows_with_amount for row in rows) < row_count
...
OBJECT_REASON_NO_CONTRACTS = "no_contracts"
OBJECT_REASON_NO_COUNTED_CONTRACT = "no_counted_contract"

#: Порядок — приоритет, сверху вниз, как и у договорной лестницы.
#:
#: «Нет договоров вовсе» и «договоры есть, но ни один не учтён» РАЗВЕДЕНЫ, и это
#: не педантизм (находка ревью Codex). Второй случай уже объяснён договорной
#: половиной охвата: договор назван там со своей причиной. Первый не объяснён
#: НИГДЕ — ни один договор о таком объекте не упоминает, — и, будучи слит со
#: вторым, он терялся: сноска рейтинга показывала только `many_contracts`, а при
#: пустой договорной половине скрывалась целиком, и объект пропадал молча.
OBJECT_REASONS: tuple[str, ...] = (
    OBJECT_REASON_MANY_CONTRACTS,
    OBJECT_REASON_NO_CONTRACTS,
...
def object_layer(db: Session, contracts: Sequence[ContractMoney]) -> list[ObjectMoney]:
    ...
    for object_id, title, area_total_sp, area_useful_sp in rows:
        ...
        elif not object_contracts:
            reason = OBJECT_REASON_NO_CONTRACTS
        elif not counted:
...
def base_counters(db: Session, contracts: Sequence[ContractMoney]) -> dict:
    ...
    * рейтинг и диаграмма этой же страницы группируют по снимку договора, и
      счётчик обязан считать признак ТОГО ЖЕ РОДА. Замерено браузерным smoke на
      стенде `gca_dev`: по классу объекта выходило «в 1 классах», тогда как
      диаграмма двумя блоками ниже рисовала ТРИ дорожки — счётчик обещал МЕНЬШЕ
      классов, чем видно на экране, и свести это было нечем;
    * пустой класс справочника на главной ничего не описывает.

    **Счётчик считает ВСЮ базу, диаграмма — свою выборку, и совпадать они не
    обязаны** (уточнение по ревью Codex; прежняя редакция этого докстринга
    обещала равенство и обещала зря). Дорожка рисуется только у учтённого
    одно-договорного объекта с площадью, поэтому договор без сметы приносит свой
    класс в счётчик и не приносит дорожку: «в 2 классах» при одной дорожке —
    законно, и охват диаграммы назван под ней отдельной строкой.

    Инвариант, который ОБЯЗАН держаться и который стерегут тесты: счётчик
    никогда не меньше числа дорожек. Обратное означало бы, что на экране видно
    больше классов, чем страница насчитала, — а это несводимое противоречие, и
    ровно оно было дефектом до правки.
    """
