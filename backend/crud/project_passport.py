...
def _rate_coverage(
    ...
    **`money_share` квантуется до сотых процента** (§2.10, ревизия
    ...
    неквантованного `money_share`: диапазон (проверка 4, ниже) проверяется на
    неквантованном значении, квантованное идёт в ответ только после того, как
    эта проверка пройдена — квантование форма ответа, а не часть правила.
    ...
    **Ноль — не `no_articles`** (DoD 25): набор может быть непустым и без
    ...
    # Проверка 4 — на РЕЗУЛЬТАТЕ, последней (см. докстроку выше): ловит выход
    # за диапазон и по причинам, не перечисленным явно, включая дефект набора.
    # Смотрит на НЕквантованное значение — квантование выше — форма ответа,
    # а не часть правила (§2.10, ревизия «money_share квантуется»).
