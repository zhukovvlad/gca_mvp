...
def diagnostics(db: Session, scope: RoundScope) -> list[dict]:
    ...
    На ЖИВОЙ структуре `unresolved_chapter_ref` не полагается на уже
    материализованный `work_category_id`: причина отказа всегда
    пересчитывается свежим `resolve_ref` (ревью после первого прохода задачи —
    доверие столбцу без пересчёта и есть тот приближённый селектор, которого
    эта функция обязана избегать; на согласованных данных оба пути совпадают,
    поэтому расхождение видно только на устаревших — ровно та подмена, которую
    нельзя ловить молчанием).

    Оговорка «на живой структуре» существенна, и первая редакция докстроки
    обещала лишнее (находка финального ревью ветки): у погашенной структуры
    ветка ниже отбирает допработы как раз по `work_category_id is None`. Там
    это не приближение, а единственный доступный признак — резолвер при
    погашенной структуре не даёт ни одной настоящей статьи и сравнивать
    причину не с чем.
