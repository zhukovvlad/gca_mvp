...
def _groups(db: Session, estimate_ids: Sequence[int]) -> dict[int, list[_RawGroup]]:
    ...
    # спека §2.7 запрещает: по абсолютам изменение объёма при неизменных долях
    # выглядело бы сменой состава.
    # `unit_works`/`unit_materials`/`unit_indirect` — ОТДЕЛЬНЫЕ
    ...
    rows = db.execute(
        sa.select(
            ...
            sa.func.sum(works).filter(sa.and_(_finite(total), _finite(works))).label("c_works"),
            sa.func.sum(materials).filter(sa.and_(_finite(total), _finite(materials))).label("c_materials"),
            sa.func.sum(indirect).filter(sa.and_(_finite(total), _finite(indirect))).label("c_indirect"),
