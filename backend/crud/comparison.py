...
from money.price import is_price
...
def _compute_median(cells: dict[int, BucketCell], ordered_ids: Sequence[int]) -> _MedianResult:
    ...
    Отбор сравнимых ячеек идёт через общий помощник `money.price.is_price`
    (план фичи предиката цены, задача 8; спека `2026-09-09-price-predicate-
    design.md` §1.1 называет ЭТУ функцию по имени как место сегодняшней
    второй копии правила, а §2.2 перечисляет медиану сравнения среди
    потребителей помощника описательно, не по имени). Раньше здесь стояла
    ВТОРАЯ, самостоятельная копия того же
    правила (`net_per_sqm is not None and net_per_sqm != 0`) — копия была
    неполна: она пропускала как «сравнимое» отрицательное и нефинитное
    значение, хотя ни то ни другое сравнивать нельзя (`docs/pitfalls/
    backend.md`: у величины, чей знак участвует в решении, нужны оба знака).
    Второй копии в этом файле больше нет.

    `net_per_sqm` — не «цена за единицу» в смысле спеки §2.1 (там это
    `unit_cost_total` ОДНОЙ позиции); здесь это нетто корзины, делённое на
    площадь объекта, — ТРЕТИЙ факт со своим носителем, рядом с ценой и весом
    (`money/price.py`: `is_weight` заведена отдельной функцией именно потому,
    что формула та же, а факт другой). `is_price` применён сюда осознанно как
    ОБЩЕЕ правило «конечно и больше нуля», а не как утверждение, что эта
    величина — цена за единицу; план задачи 8 не заводит для неё отдельного
    имени и не просит переименовывать факт, только снять вторую копию
    правила. Следствие названо в докстроке самого `is_price`: правка правила
    ЦЕНЫ теперь молча меняет и правило СРАВНЕНИЯ договоров — эта функция
    читает `is_price`, а не отдельную копию.

    Исключённая ячейка не получает диагноза причины: предикат называет только
    сам факт «непригодна для сравнения» — пусто, ноль, отрицательное или
    нефинитное неразличимы на этом уровне, — а не почему именно, будь то «нет
    ТЭП», «работ нет либо учтены в другой статье» или ошибка ввода. Это то же
    решение, каким `is_price` не называет причину отсутствия цены
    (`money/price.py`).

    Меньше трёх сопоставимых -> `value is None` (правило 5, DoD 11): подсветки
    в строке нет вовсе, но счётчик и список id всё равно возвращаются — экрану
    нужно число для «сопоставимых меньше трёх», а не молчаливо пустая строка.
    """
    comparable = [
        (contract_id, cells[contract_id].net_per_sqm)
        for contract_id in ordered_ids
        if is_price(cells[contract_id].net_per_sqm)
    ]
...
def _apply_deviation(
    ...
    `median.contract_ids` (пустые, нулевые, отрицательные, нефинитные или
    отсутствующие — после задачи 8 `_compute_median` отфильтровывает их ВСЕ
    одним предикатом `is_price`, не только ноль) отклонения не получают:
    «Нераспределённое» и подобные им сюда просто не передаются (вызывающий
    код решает это заранее, до вызова медианы), а всё непригодное в ЭТОЙ
    строке уже отфильтровано `_compute_median`.
    """
    if median.value is None or median.value == 0:
        ...
        #
        # Через `_compute_median` эта ветка СЕГОДНЯ недостижима (задача 8,
        # ревью): `is_price` требует от каждого сравнимого значения быть
        # конечным и строго положительным, поэтому медиана — само одно из
        # положительных значений (нечётная выборка) либо среднее ДВУХ
        # положительных под `_DIV_CONTEXT` (prec=100, Emin=-999999) — точным
        # нулём выйти не может ни на одном достижимом входе (замерено, не
        # выведено: см. `test_comparison_buckets.py::
        # test_apply_deviation_is_a_noop_only_when_the_median_is_zero`).
        # СТАРОЕ обоснование этой ветки — пара «-x/+x» из отрицательного и
        # положительного нетто — своим же предикатом и снято: отрицательная
        # ячейка в `median.contract_ids` теперь не попадает вовсе.
        #
        # Условие остаётся: `_apply_deviation` — код общего назначения, а не
        # только сегодняшнего вызывающего, и «недостижимо сегодня» не значит
        # «недостижимо никогда» (`docs/insights/verifying-guards.md`, слой
        # 10). Без него будущий вызывающий с нулевой медианой уронил бы
        # строку `DivisionByZero`-ошибкой вместо прочерка. Строка просто
        # остаётся без подсветки; заметку «меньше трёх сопоставимых» экран
        # при этом НЕ покажет — она читает `comparable_count`, а он верен.
        return cells
    result = dict(cells)
