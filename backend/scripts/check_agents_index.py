...
13. состав маршрутизатора §7 против `EXPECTED_SCREEN_ANCHORS` — ТРИ колонки
    ...
    и биекция: пара «номер → якорь» сверяется ПОЗИЦИОННО (перестановка двух
...
о структуре: check_13 сочетает ПОЗИЦИОННЫЙ режим (пара «номер → якорь»),
...
def screen_router_pairs(lines: list[str]) -> list[tuple[str, str | None]]:
    """Пары «номер пункта → якорь первой ссылки на screens.md» маршрутизатора §7.
...
def check_13(lines: list[str]) -> tuple[bool, list[str]]:
    ...
    2. пунктов маршрутизатора не восемь — кратность, границы 7 и 9, ПЛЮС
       ...
       жирного маркера);
    3. пара «номер → якорь» на своей позиции не совпала с ожиданием — ОДИН
