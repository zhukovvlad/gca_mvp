...
МНОЖЕСТВА, а не их мощности. Так работают проверки 1, 5 и 7. ЧЕТЫРЕ стража
дубликатов сделаны исключениями намеренно и названы: проверка 2 сравнивает
мощности строк указателя, проверка 4 считает объявления по списку, проверка 8 —
строки маршрутизации, проверка 9 ищет повтор пункта по всему корпусу; множество
как раз и скрывает дубликат, который все четыре ищут. Остальные (3, 6, 10, 11) —
построчные условия и коллекций не сопоставляют вовсе.
...
* **полноту текста архивной записи.** Проверка 5 держит ЗАГОЛОВОК `## v6.N`, а
  не тело: запись, у которой вырезали весь текст и оставили заголовок, проходит
  стража зелёной — предъявлено снятием на 28 строках записи `v6.10`. Держится
  только то, что версия вообще объявлена: которой в архиве нет, та не
  разрешается ни в одном упоминании. Полнота переноса текста — чтение на ревью,
  как и полнота переезда граблей ниже;
