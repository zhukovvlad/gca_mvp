...
12. §2 ссылается на существующий файл источников; у файла ровно одна шапка
    ...
    `AGENTS.md`. Пять условий — **шесть** отдельных входов: у требования «ровно
    три SHA» границы две, и предъявлять надо каждую. Первая редакция условия
    была `< 3`, то есть держала только нижнюю: файл с ЧЕТЫРЬМЯ строками
    репозиториев проходил зелёным, хотя источников три.
...
def check_12(lines: list[str]) -> tuple[bool, list[str]]:
    ...
    Пять условий и ШЕСТЬ отдельных входов: маршрут в никуда; файл без шапки
    «когда читать»; строк с SHA меньше трёх; строк с SHA больше трёх; нет
    разделителя таблицы (или он не о четырёх колонках); §-адрес без квалификации
    `AGENTS.md`. Разнесены потому, что вход, ломающий ровно одно, обязан
    существовать для каждого свойства — иначе предъявить проверку красной по
    свойству нельзя.

    Входов шесть, а не пять, потому что «ровно три SHA» — требование
    ДВУСТОРОННЕЕ. Первая редакция писала `< 3` и держала одну сторону; лишняя
    четвёртая строка проходила зелёной, и отрицательное предъявление это
    пропустило, потому что мутировало только в сторону удаления.
