"""Указатель §12, ревизии, архив, маршрутизатор §11 → `docs/pitfalls/` и
маршрутизаторы §4/§7/§10 → `docs/reference/` — 16 проверок.
...
* **что относительные ссылки внутри `docs/reference/*.md` разрешаются в
  ...
  прошёл бы все шестнадцать проверок зелёными. Измерено: на коммите этой
