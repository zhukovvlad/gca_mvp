...
МНОЖЕСТВА, а не их мощности. Так работают проверки 1, 5 и 7. Три стража
кратности сделаны исключениями намеренно и названы: проверка 2 сравнивает
мощности, проверка 4 считает объявления по списку, проверка 8 — строки
маршрутизации; множество как раз и скрывает дубликат, который они ищут.
Остальные (3, 6, 9, 10, 11) — построчные условия и коллекций не сопоставляют
вовсе.
...
* **состав и форму строк исторических ревизий.** С v6.19 их нет в преамбуле
  ...
  ослабление стража, а исчезновение того, что он стерёг; сами ревизии
  по-прежнему обязаны лежать в архиве полным текстом, и это держит проверка 5:
  версия, которой там нет, не разрешается ни в одном упоминании;
...
def main() -> int:
    ...
    archive_declared = {version for version, _ in archive_entries(archive_lines)}
