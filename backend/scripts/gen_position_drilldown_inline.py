...
#: файле»: в показанных нетто-величинах собственные суммы 10.6 идут
#: 69,8 → 67,7 → 0 → 0, а поддерево 69,8 → 74,6, потому что работы съехали
#: в 10.6.1 и 10.6.3.
ARTICLE_MOVE = "10.6"
ROOT_MOVE = "10"
...
#: «подешевела на 3 %», и стоит вопроса к подрядчику независимо от суммы
#: (решение пользователя 30.08.2026); на хвосте таких строк бывает много, и
#: там остаток сворачивается. Потолок 10 взят по замеру 30.08.2026 ПОСЛЕ
#: перехода на поддерево и на втором тендере: медиана таких строк на статью
#: 0 и 1, девятая дециль 6 и 9 (блок замеров) — то есть десять покрывают
#: почти все статьи целиком. Исторический потолок 5 стоял на девятой децили
#: 3 — замере собственных строк одной трассы, которого больше нет.
PARTIAL_CAP = 10
...
def subtree_ids(cur, code: str) -> list[int]:
    ...
    в показанных нетто-величинах 69,8 → 67,7 → 0 → 0, а поддерево 69,8 → 74,6:
    разложение по собственным строкам написало бы «нет в файле» на каждой
    работе под строкой «+7,0 %».
...
#: сквозных групп 80 % против 58 %, объём двигался у 49 % объяснителей
#: против 12 %.
