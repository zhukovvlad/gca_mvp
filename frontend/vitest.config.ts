...
export default defineConfig({
  ...
  test: {
    // `threads` вместо дефолтных `forks`. Замер 2026-09-02 на полном наборе:
    // forks — 92,0 / 98,6 / 100,2 с, threads — 80,7 / 92,7 с. Выигрыш около
    // 10 %, и он НЕ влияет на `just ci`: там фронт-цепочка идёт параллельно
    // бэкенду и целиком уходит в его тень. Ставится ради точечных прогонов и
    // `just test-frontend`.
    //
    // `isolate` НЕ выключать. Замерено там же: `--no-isolate` даёт 75 с, но
    // роняет 177 тестов в 24 файлах — состояние протекает между файлами. Это
    // не «флаки», а отсутствие изоляции, ради которой окружение и поднимается.
    pool: "threads",
    environment: "jsdom",
    globals: true,
