...
describe("Таблица статей: единица, объём, ставка ₽/ед.", () => {
  ...
  it("узел rate показывает единицу, объём и ставку — без подсказки: сервер уже округлил", async () => {
    // Фикстура: «01.01» «Разработка грунта» — единственный узел с rate_state
    // "rate" (unit "м³", volume "3000.00", unit_rate "166.67" — сервер
    // квантует ставку безусловно, §2.10, поэтому по проводу приходит уже
    // округлённое значение, а не хвост `_RATE_CONTEXT`). Child узла «01» —
    ...
    // Ставка — ЕДИНСТВЕННОЕ хранимое денежное поле экрана, у которого нет
    // «точного» варианта отдельно от показанного: округлённое и точное
    // значение здесь совпадают (сервер уже отдал 166.67), поэтому `MoneyCell`
    // не кладёт `title` вовсе (см. её докстроку: подсказка — только когда
    // округление действительно что-то изменило). Это отличает ставку от
    // ₽/м² и от доли, которые сервер НЕ квантует и где подсказка есть.
    expect(within(rate).queryByTitle(/Точное значение/)).not.toBeInTheDocument();
  });
    ...
    it("набор непуст, ставок нет, суммы полные (complete) → печатает «покрыто 0,00 %» и называет M = 1", async () => {
      withPassport((base) => ({
        ...
        rate_coverage: {
          ...
          articles_total: 1,
          // "0.00", а не "0": сервер квантует `money_share` безусловно
          // (§2.10, ревизия «money_share квантуется») — по проводу ноль
          // приходит уже с двумя знаками после запятой.
          money_share: "0.00",
