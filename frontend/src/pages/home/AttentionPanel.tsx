...
export function AttentionPanel({ data }: { data: DashboardAttention }) {
  ...
  if (shown.length === 0) {
    ...
    // показываются: «расхождений нет» и список нулей — два разных сообщения, и
    // вместе они противоречат друг другу.
    //
    // ФОРМУЛИРОВКА СУЖЕНА ПРОТИВ МАКЕТА (спека §2.5, находка ревью Codex).
    // Макет писал «Всё, что база умеет проверить, сходится» — утверждение обо
    // ВСЕЙ базе, тогда как таб проверяет ровно пять вещей. Объект без договоров
    // ни одной из них не виден, и сноска рейтинга на соседнем табе в этот самый
    // момент пишет «1 объект без договоров»: два экрана утверждали бы
    // противоположное. Панель называет СВОЙ охват и отсылает туда, где живёт
    // остальное.
    //
    // Отсылка говорит про ПРИЧИНЫ ИСКЛЮЧЕНИЯ ИЗ РАСЧЁТА, а не про «всё, что не
    // вошло в рейтинг» (уточнение второго круга ревью Codex). Рейтинг
    // показывает топ-10, и одиннадцатый объект в сноску не попадает НАМЕРЕННО:
    // он учтён, просто не показан, — сноска про исключённых, а не про
    // непоказанных (`object_ranking`). Прежняя редакция обещала и его.
    return (
      <div
        data-testid="attention-ok"
        className="rounded-lg border border-border-subtle bg-surface px-4 py-4 text-sm text-fg-secondary"
      >
        Пять проверок базы не нашли расхождений. Причины исключения из расчёта рейтинга
        названы сноской под ним на основном дашборде.
      </div>
    );
  }
