...
function ExcludedNote({
  ...
  return (
    ...
    >
      ...
      {isAdmin && (
        // «ЧАСТЬ причин», а не «причины» (спека §2.5, находка ревью Codex).
        // Состав второго таба закреплён пятью строками решения 12, и
        // `no_contracts` среди них нет: объект без договоров ни одной из пяти
        // проверок не виден. Безусловное «Причины — во вкладке» отправляло бы
        // читателя туда, где его причины не окажется, — а при остальных нулевых
        // счётчиках таб вдобавок сказал бы «расхождений нет».
        <span> Часть причин разбирается во вкладке «На что обратить внимание».</span>
      )}
    </div>
  );
}
