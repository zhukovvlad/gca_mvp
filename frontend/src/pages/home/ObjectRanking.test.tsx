...
describe("Таб «На что обратить внимание»", () => {
  ...
  it("пустое состояние называет СВОЙ охват, а не всю базу", async () => {
    ...
    // Отсылка — про ПРИЧИНЫ ИСКЛЮЧЕНИЯ из расчёта, а не про «всё, что не вошло
    // в рейтинг»: одиннадцатый объект учтён и в сноску не попадает намеренно,
    // так что широкая формулировка обещала бы и его (второй круг ревью Codex).
    expect(ok).toHaveTextContent("Причины исключения из расчёта рейтинга");
    expect(ok).not.toHaveTextContent("не вошедшие в рейтинг");
