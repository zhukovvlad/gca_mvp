  ...
  return (
    <div className="flex flex-1 flex-col items-center gap-1 min-w-0">
      ...
      {/*
        ...
        `dashOnNone` — по просмотру на стенде 28.08.2026, решение пользователя:
        у ПЕРВОЙ выбранной колонки изменения нет по построению (`kind = 'none'`,
        `reason = 'first_column'` — сравнивать не с чем), и пустое место на
        месте значка читалось как «значение почему-то не показали». Прочерк
        говорит то же, что уже говорит KPI «Последний к первому» на пустом
        входе. Побочно он уравнивает высоту стеков колонок — но выравнивание
        столбиков держится не на этом, а на `items-start` строки ниже: у
        прочерка своя причина, у выравнивания своя.

        Причина `first_column` — не единственная достижимая: колонка, идущая
        ПОСЛЕ колонки с неизвестной базой НДС, тоже получает `kind = 'none'`,
        но с `reason = 'unknown_vat_base'` (`_total_change_step` берёт
        `reason or prev_reason`; фикстура `stageSummaryWithUnknownSecondColumn`).
        Там прочерк уместен по той же причине — шага нет, — а саму причину
        несёт `title` значка.
      */}
      <div className={TRACK_TEXT_CLASS}>
        <ChangeBadge change={column.total_change} dashOnNone />
      </div>
    </div>
  );
}
...
export function StageSummaryTrack({ summary }: { summary: StageSummary }) {
  ...
  /*
    `items-start`, а НЕ `items-end` (замер на стенде 28.08.2026). Колонка — это
    вертикальный стек «контейнер столбика 150 px → сумма → подпись этапа →
    значок изменения», и выравнивание строки двигает СТЕК ЦЕЛИКОМ, а не
    столбики. Стеки бывают разной высоты (у первой колонки значок изменения был
    пуст; сумма или подпись могут перенестись на вторую строку в узкой
    колонке), и при выравнивании по низу более короткий стек уезжал вниз весь:
    замерено 22,39 px разницы у первой колонки при 4 этапах в окне 1440 px —
    её столбик стоял ниже соседних, а сумма и подпись на строку ниже соседних.
    При `items-start` контейнеры столбиков начинаются на одной высоте, а раз
    высота контейнера фиксирована (150 px), совпадают и их низы — то есть
    основания столбиков, от которых читается высота. Разница высот стеков
    уходит ВНИЗ, под подписи, где ничего не кодирует.

    Это НЕ про высоту самих столбиков: она задаётся процентом от контейнера
    (`bar_height_pct`) и от выравнивания не зависит — проверено сверкой
    измеренных долей с полем ответа. Проверять правку можно только браузером:
    jsdom раскладку не считает (`docs/insights/unobservable-in-the-runner.md`).
  */
  return (
