...
export default function ContractCardPage() {
   ...
   * объекта рядом с ним дал бы два поля с одним именем и разным смыслом. Хук
   * вызывается безусловно (до ранних `return`), поэтому объект неопределён,
   * пока карточка договора не загружена — 0 — заведомо несуществующий id, а
   * запрос по нему тихо уходит в 404 и просто не используется ниже.
   */
  const objectQ = useObject(contract?.object_id ?? 0);
