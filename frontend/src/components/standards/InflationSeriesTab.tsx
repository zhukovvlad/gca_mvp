...
/**
 * Ряды индексов инфляции — третья вкладка экрана «Нормативы» (`AGENTS.md` §7 п. 3
 * в редакции v6.10; спека 2026-08-18 §2.10).
 *
 * Здесь, а не отдельным экраном: §7 п. 3 отдаёт этому экрану «классы, ставки,
 ...
 * и вернуть ряд можно только со списка, где он виден. В СЕЛЕКТОР выбора на
 * `/compare` архивные при этом не попадают — там список запрашивается без
 * `include_archived` (§2.10).
 */
