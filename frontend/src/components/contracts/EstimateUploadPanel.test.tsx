...
describe("Упавший рефетч карточки не открывает диалог на непроверенном числе (находка ревью, finding 3)", () => {
  it("409 при упавшем GET /contracts/:id — отказ вместо диалога, повтор загрузки остаётся возможным", async () => {
    ...
    // Порядок утверждений здесь ЗНАЧИМ, и это не стилистика (проверено снятием
    // защиты контролирующим). Открытый `AlertDialog` модален: он помечает
    // фон `aria-hidden`, и `findByRole("alert")` перестаёт видеть текст отказа
    // ровно потому, что диалог открылся. Поэтому при снятой защите прогон
    // падал бы на поиске отказа — то есть на утверждении о ТУПИКЕ, — и до
    // утверждений о самом диалоге не доходил бы никогда. Сначала проверяется
    // главный вред (диалог на непроверенном числе), и только потом — что
    // аналитик не остался без выхода.
    await waitFor(() => expect(screen.getByTestId("upload-rejection")).toBeInTheDocument());
