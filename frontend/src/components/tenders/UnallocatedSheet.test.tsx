...
describe("UnallocatedSheet — Sheet-верстак этапного разноса раунда (§2.7)", () => {
  ...
  it("подсказки блоков — полный текст макета, а не сокращённая форма плана", async () => {
    ...
    expect(
      screen.getByText(
        "Файловый порядок ведомости; счётчик «N позиций» — сколько строк сдвинет " +
          "решение на этом разделе: строки под вложенными разделами, у которых своя " +
          "статья в файле, оно не двигает, а разделы, где двигать нечего, здесь не " +
          "показываются вовсе. Здесь три из четырёх состояний " +
