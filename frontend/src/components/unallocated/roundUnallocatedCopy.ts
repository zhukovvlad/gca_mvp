 ...
 *
 * `PENDING_HINT` правлен 02.09.2026 вместе с макетом и §2.3 спеки: прежняя
 * редакция описывала «N позиций» как полный размер файлового поддерева, а
 * счётчик стал охватом решения (спека §2.2, «Достижимость»). Макет и этот
 * файл менялись ОДНИМ коммитом намеренно — расхождение с макетом следующее
 * ревью прочло бы как дефект кода и вернуло бы прежний текст.
 *
