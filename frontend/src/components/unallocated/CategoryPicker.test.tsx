...
describe("CategoryPicker: данные пришли при ОТКРЫТОМ поповере", () => {
  ...
  */
  ...
  it("новый массив с тем же составом НЕ трогает выбор пользователя", async () => {
    /*
      ...
      Массив здесь НОВЫЙ, а состав и порядок — прежние: именно это и
      проверяется. Первая редакция называлась «тот же набор в другом порядке
      обхода» и обещала больше, чем делает (находка внешнего ревью).

      Настоящая перестановка — тот же состав, другой порядок первого появления
      — ключ бы изменила, и сброс произошёл бы. Сегодня она недостижима:
      заметки приходят из `Classification.notes` бэкенда, а он обходит сметы
      радиуса стабильным `estimate_id ASC`, так что при неизменном составе
      порядок неизменен тоже. Ключ намеренно оставлен чувствительным к
      порядку — делать его нечувствительным значило бы писать код под
      недостижимый случай.
    */
    const user = userEvent.setup();
    const { onPick, rerenderWith } = renderPicker(["а", "б"]);
