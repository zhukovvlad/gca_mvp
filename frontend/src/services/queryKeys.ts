...
export const qk = {
  ...
  passport: {
     ...
     *
     * Фаза 6, уходит в задаче 11 (заменяется `project` ниже).
     */
    one: (contractId: number) => ["passport", contractId] as const,
    /**
     * Паспорт проекта по статьям классификатора (Ф6 фазы 7, задача 6).
     *
     * Корень НЕ заводится заново — `["passport"]` уже является префиксом ОБОИХ
     * ключей (`one` и `project`), поэтому инвалидация `qk.passport.all`,
     * которая уже стоит в `useUpdateObject` (пять корней, F5) и
     * `useUpdateAppSettings`, накрывает новый паспорт проекта ПО ПОСТРОЕНИЮ —
     * без правки списка инвалидации. Так обязательство 1, унаследованное из
     * F5 («правка объекта обязана перерисовать всё, где лежит его название»),
     * закрывается для Ф6 бесплатно: свой отдельный корень (например
     * `qk.projectPassport`) выглядел бы так же удобно, но молча выпал бы из
     * этой инвалидации, и ни один существующий тест F5 этого не заметил бы —
     * см. `queries.test.tsx`, тест «корень паспорта переиспользован новым
     * хуком».
     */
