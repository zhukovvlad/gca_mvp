...
- **[Прогон, который молчит вместо того чтобы упасть](docs/insights/silent-test-runs.md)** — успех читается по числу прошедших, а не по коду возврата: файл без `pytestmark` (молча пропало 65 тестов), integration без `TEST_DATABASE_URL`, `&&` с `grep` вместо отдельных шагов, невзлетевший раннер, снесённый `caplog`.
