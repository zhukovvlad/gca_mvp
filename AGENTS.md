...
4. **Паспорт объекта** — новая страница, А4, `@media print`, PDF в MVP = Ctrl+P. Состав: реквизиты договора (номер, подписант, дата, сумма, класс), ключевые расценки, отклонения с подсветкой. **Правило «ключевых расценок»:** топ-15 позиций последней сметы по `total_cost_total`; N хранится в БД (таблица настроек, экран Settings из udp-tenders), не в конфиге приложения. **N ограничен сверху** (`CHECK` в миграции 0004, диапазон 1–20): требование §10 «печатается на одну страницу А4» держится по построению, а не обрезкой списка при печати — обрезка означала бы, что форма молча теряет часть того, что сама объявила своим составом. Граница подобрана замером печатной раскладки, а не расчётом (`docs/phase6-analytics.md` §1.5, §3.2).
...
- Паспорт печатается на одну страницу А4 со всеми полями §1 пункт 2 (номер договора, подписант, дата, ключевые расценки, отклонения); состав тот же в §7.4. Прежняя редакция ссылалась на «§1.2» — подраздела с таким номером в документе нет.
